...
PUBLIC ASSIGN_FILE_NAMES,INITIALIZE_GLOBAL_DUMPS,INITIALIZE_MESH_DUMPS,WRITE_STATUS_FILES, &
       ...
       DUMP_UVW,DUMP_GEOM,UPDATE_DEVICES_2, PROFILE_AVERAGE
...
ALLOCATE(FN_MOISTURE(NMESHES,5))
ALLOCATE(LU_MOISTURE(NMESHES,5))
ALLOCATE(FN_SPEC_HUM(NMESHES,5))
ALLOCATE(LU_SPEC_HUM(NMESHES,5))
ALLOCATE(FN_TEMP_A(NMESHES,5))
ALLOCATE(LU_TEMP_A(NMESHES,5))
ALLOCATE(FN_TEMP_M(NMESHES,5))
ALLOCATE(LU_TEMP_M(NMESHES,5))
ALLOCATE(FN_LIQ(NMESHES,5))
ALLOCATE(LU_LIQ(NMESHES,5))
ALLOCATE(FN_TEMP_L(NMESHES,5))
ALLOCATE(LU_TEMP_L(NMESHES,5))
...
! !IF (NM .LE. 200) then
	...
	! IF (mod(ITM,50) == 0) then
		! IF ((T-ITM) .LE. 0.1) THEN
		! WRITE(FN_PROFAVG(NM),'(A,A,I0,A,I0,A)') TRIM(CHID),'_profavg_',NM,'_',ITM,'.txt'
			...
			! DO K = 0,KBP1
				! DO J = 0,JBP1
					! DO I = 0,IBP1
						! WRITE(LU_PROFAVG(NM),*) ICYC, T, X(I), Y(J), Z(K), M%AVG_T(I,J,K), M%AVG_U(I,J,K), M%AVG_V(I,J,K), M%AVG_W(I,J,K), &
