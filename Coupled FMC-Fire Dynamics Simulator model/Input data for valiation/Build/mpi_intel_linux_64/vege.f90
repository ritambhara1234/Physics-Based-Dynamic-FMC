...
SUBROUTINE LEVEL_SET_FIRESPREAD(T,DT,NM,ICYC)

USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: NM, ICYC
INTEGER :: IIG,IW,JJG,IC,data_tot
...
REAL(EB), ALLOCATABLE, DIMENSION(:) :: ROS_NO_WIND_NO_SLOPE_WITH_MOISTURE_VARIATION
REAL(EB), ALLOCATABLE, DIMENSION(:,:) :: VEG_LSET_ROS_00
INTEGER :: NX1,NY1, SURF_INDEX, ROTHERMAL_FUEL_INDEX
...
ALLOCATE (ROS_NO_WIND_NO_SLOPE_WITH_MOISTURE_VARIATION(data_tot))

ALLOCATE (VEG_LSET_ROS_00(IBAR,JBAR))

DO SURF_INDEX=0,N_SURF
SF => SURFACE(SURF_INDEX)
    IF (SF%VEG_LSET_SPREAD .AND. SF%VEG_LSET_FUEL_INDEX>0) THEN
	ROTHERMAL_FUEL_INDEX=SF%VEG_LSET_FUEL_INDEX

	CALL ROS_NO_WIND_NO_SLOPE_UPDATED_WITH_MOISTURE(ROTHERMAL_FUEL_INDEX,SURF_INDEX, ICYC, NM, &
									ROS_NO_WIND_NO_SLOPE_WITH_MOISTURE_VARIATION)
    ELSE 
	
	ROS_NO_WIND_NO_SLOPE_WITH_MOISTURE_VARIATION(data_tot) = 0.2;
...
DO JJG = 1,JBAR
	DO IIG = 1,IBAR
        NY1 = (JJG-1)*NX1 + IIG;
	    VEG_LSET_ROS_00(IIG,JJG) = ROS_NO_WIND_NO_SLOPE_WITH_MOISTURE_VARIATION(NY1)		
	END DO
END DO
...
DO JJG=1,JBAR
   DO IIG=1,IBAR
      ...
      IF (VEG_LSET_ROS_00(IIG,JJG) > 0._EB) ROS_HEAD(IIG,JJG) = VEG_LSET_ROS_00(IIG,JJG)*(1._EB + PHI_WS(IIG,JJG))
...
REAL(EB) :: w0d1, w0d2, w0d3, w0lh, w0lw, md2, md3, mlh, mlw, svd1, svd2, svd3, svlh, svlw, depth, rhop, heat, st, se, mx
REAL(EB) :: swd1, swd2, swd3, swlh, swlw, swd, swl, swt, s2wt, sw2d, sw2l, swml, sigma, rhob, beta, &
            betaOpt, wnd, wnl, hnd1, hnd2, hnd3, hnlh, hnlw, hnd, hnl, bigW, hnmd, mfdead, mxlive, rml, etaMl, &
            etas, gammaMax, bigA, gamma, xi, epsd1, epsd2, epsd3, epslh, epslw, bigQd1, bigQd2, &
            bigQd3, bigQlh, bigQlw, hskz, hsk,  md1, swmd, rmd, etaMd, etaM, bigIr
TYPE(SURFACE_TYPE), POINTER :: SF
...
SUBROUTINE ROS_NO_WIND_NO_SLOPE_UPDATED_WITH_MOISTURE(ROTHERMEL_FUEL_INDEX,SURF_INDEX, ICYC, NM, &
									ROS_NO_WIND_NO_SLOPE_WITH_MOISTURE_VARIATION)

INTEGER, INTENT(IN) :: ROTHERMEL_FUEL_INDEX,SURF_INDEX, ICYC, NM
REAL(EB), ALLOCATABLE, DIMENSION(:), INTENT(OUT) :: ROS_NO_WIND_NO_SLOPE_WITH_MOISTURE_VARIATION
REAL(EB) :: w0d1, w0d2, w0d3, w0lh, w0lw, md2, md3, mlh, mlw, svd1, svd2, svd3, svlh, svlw, depth, rhop, heat, st, se, mx
REAL(EB) :: swd1, swd2, swd3, swlh, swlw, swd, swl, swt, s2wt, sw2d, sw2l, swml, sigma, rhob, beta, &
            betaOpt, wnd, wnl, hnd1, hnd2, hnd3, hnlh, hnlw, hnd, hnl, bigW, &
            etas, gammaMax, bigA, gamma, xi, epsd1, epsd2, epsd3, epslh, epslw, bigQd2, &
            bigQd3, bigQlh, bigQlw
REAL(EB), ALLOCATABLE, DIMENSION(:) :: md1, swmd, rmd, etaMd, etaM, bigIr, mass_flux_all, hnmd, mfdead, mxlive, rml, & 
										etaMl, bigQd1, hskz, hsk 
REAL(EB), ALLOCATABLE, DIMENSION(:,:) :: m_at_old_t
INTEGER :: data_tot,i
...
ALLOCATE (md1(data_tot))
ALLOCATE (swmd(data_tot))
ALLOCATE (rmd(data_tot))
ALLOCATE (eTaMd(data_tot))
ALLOCATE (etaM(data_tot))
ALLOCATE (bigIr(data_tot))

ALLOCATE (mass_flux_all(data_tot))
ALLOCATE (hnmd(data_tot))
ALLOCATE (mfdead(data_tot))
ALLOCATE (mxlive(data_tot))
ALLOCATE (rml(data_tot))
ALLOCATE (etaMl(data_tot))
ALLOCATE (bigQd1(data_tot))
ALLOCATE (hskz(data_tot))
ALLOCATE (hsk(data_tot))

ALLOCATE (m_at_old_t(data_tot,5))
ALLOCATE (ROS_NO_WIND_NO_SLOPE_WITH_MOISTURE_VARIATION(data_tot))
...
! if (NM == 1) then
 ...
 ! open (0222334, file='Raasch_Mois_1_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)
 
! ELSEIF (NM == 2) then

! open (0222334, file='Raasch_Mois_2_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)
 
! ELSEIF (NM == 3) then

! open (0222334, file='Raasch_Mois_3_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 4) then

! open (0222334, file='Raasch_Mois_4_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 5) then

! open (0222334, file='Raasch_Mois_5_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 6) then

! open (0222334, file='Raasch_Mois_6_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 7) then

! open (0222334, file='Raasch_Mois_7_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 8) then

! open (0222334, file='Raasch_Mois_8_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 9) then

! open (0222334, file='Raasch_Mois_9_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 10) then

! open (0222334, file='Raasch_Mois_10_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 11) then

! open (0222334, file='Raasch_Mois_11_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 12) then

! open (0222334, file='Raasch_Mois_12_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 13) then

! open (0222334, file='Raasch_Mois_13_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 14) then

! open (0222334, file='Raasch_Mois_14_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 15) then

! open (0222334, file='Raasch_Mois_15_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 16) then

! open (0222334, file='Raasch_Mois_16_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 17) then

! open (0222334, file='Raasch_Mois_17_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 18) then

! open (0222334, file='Raasch_Mois_18_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 19) then

! open (0222334, file='Raasch_Mois_19_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 20) then

! open (0222334, file='Raasch_Mois_20_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 21) then

! open (0222334, file='Raasch_Mois_21_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 22) then

! open (0222334, file='Raasch_Mois_22_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 23) then

! open (0222334, file='Raasch_Mois_23_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 24) then 

! open (0222334, file='Raasch_Mois_24_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)

! ELSEIF (NM == 25) then

! open (0222334, file='Raasch_Mois_25_5400.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
 ! read (0222334,*) m_at_old_t
 ! close (0222334)
...
!swmd = swd1*md1 + swd2*md2 + swd3*md3  
do i = 1,data_tot
swmd(i) = swd1*md1(i) + swd2*md2 + swd3*md3 
end do
swml = swlh*mlh + swlw*mlw
...
do i = 1, data_tot
hnmd(i)   = hnd1*md1(i) + hnd2*md2 + hnd3*md3
mfdead(i) = hnmd(i)/hnd
end do
...
do i = 1,data_tot
mxlive(i) = 2.9*bigW*(1.0 - (mfdead(i)/mx)) - 0.226
end do
...
do i = 1,data_tot
etaMd(i) = 1.0 - (2.59*rmd(i)) + (5.11*rmd(i)**2) - (3.52*rmd(i)**3) 
etaMl(i) = 1.0 - (2.59*rml(i)) + (5.11*rml(i)**2) - (3.52*rml(i)**3) 
end do
...
do i = 1,data_tot
etaM(i)  = wnd*etaMd(i) + wnl*etaMl(i)
end do
...
do i = 1,data_tot  
hskz(i) = svd1*w0d1*epsd1*bigQd1(i) + svd2*w0d2*epsd2*bigQd2 + svd3*w0d3*epsd3*bigQd3 + svlh*w0lh*epslh*bigQlh + svlw*w0lw*epslw*bigQlw
hsk(i)  = rhob*hskz(i)/swt
end do
...
do i = 1,data_tot
bigIr(i) = gamma*heat*etas*etaM(i)
end do

do i = 1,data_tot
mass_flux_all(i) = bigIr(i)/heat;
end do

IF (LEVEL_SET_COUPLED_FIRE) THEN
   SF%MASS_FLUX(REACTION(1)%FUEL_SMIX_INDEX) = bigIr(1)/heat
   SF%BURN_DURATION = 756._EB/SF%VEG_LSET_SIGMA   ! Albini (Eq. 14)
ENDIF
...
do i = 1,data_tot
ROS_NO_WIND_NO_SLOPE_WITH_MOISTURE_VARIATION(i) = (bigIr(i)*xi)/hsk(i)
end do

END SUBROUTINE ROS_NO_WIND_NO_SLOPE_UPDATED_WITH_MOISTURE
