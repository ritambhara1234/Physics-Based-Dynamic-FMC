...
REAL(EB), ALLOCATABLE, DIMENSION(:,:) :: DNI_input, RelH_input
...
! Reading variables for moisture model
allocate(DNI_input(90000,1));
allocate(RelH_input(90000,1));

open (110223, file='Height.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
read (110223,*) Hei
close (110223)

open (100012, file='Final_DNI.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
read (100012,*)  DNI_input
close (100012)

open (100013, file='Final_RelH.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
read (100013,*)  RelH_input
close (100013)
...
! Assign value to profile data storage
! DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
	! CALL PROFILE_AVG_DATA(DT,NM,AVG_T, AVG_U, AVG_V, AVG_W, AVG_TW, AVG_TU, AVG_TV, &
...
MAIN_LOOP: DO
   ...
   CHANGE_TIME_STEP_LOOP: DO
      ...
      COMPUTE_DENSITY_LOOP: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
         ...
         IF (LEVEL_SET_MODE>0) CALL LEVEL_SET_FIRESPREAD(T,DT,NM,ICYC)
   ...
   COMPUTE_DIVERGENCE_2: DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
      ...
      IF (ANY(SPECIES_MIXTURE%CONDENSATION_SMIX_INDEX>0)) CALL CONDENSATION_EVAPORATION(T,DT,NM)
   ...
   ! Calling moisture model for all NM
    DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
    CALL DUMP_TEMP(T,DT,NM,DNI_input,RelH_input,ICYC,Hei)
    ENDDO
	
	DO NM=LOWER_MESH_INDEX,UPPER_MESH_INDEX
	CALL PROFILE_AVERAGE(T,DT,NM,ICYC)
	ENDDO
