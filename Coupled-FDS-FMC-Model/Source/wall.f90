...
IF (SOLID_PARTICLES) THEN
   DO IP = 1, NLP
	  ...
	  !print*, T, NM, ONE_D%IIG, ONE_D%JJG, ONE_D%KKG, IP
      IF (LPC%SOLID_PARTICLE .OR. LPC%MASSLESS_TARGET) THEN  ! Target particles are included to get gas phase values
		...
		! print*, T, NM, IP, ONE_D%IIG, ONE_D%JJG, ONE_D%KKG, ONE_D%TMP_G, ONE_D%U_TANG
		 ...
		 !print*, 'SF%THERMAL_BC_INDEX', NLP, SF%THERMAL_BC_INDEX
      ENDIF
   ENDDO
ENDIF
...
!print*, 'one'
   ...
   IF (SF%PYROLYSIS_MODEL==PYROLYSIS_PREDICTED) THEN
	...
	!  print*, 'PYROLYSIS_PREDICTED', T, NM, IP, SF%INNER_RADIUS
   ELSE
      RADIUS = SF%INNER_RADIUS + SF%THICKNESS
	!  print*, 'ELSE', T, NM, IP, SF%INNER_RADIUS
   ENDIF
   ...
   IF (LPC%DRAG_LAW /= SCREEN_DRAG .AND. LPC%DRAG_LAW /= POROUS_DRAG) THEN
      SELECT CASE(SF%GEOMETRY)
         CASE(SURF_CARTESIAN)
			...
			!print*, 'cartesian', T, NM, IP, ONE_D%AREA
...
!print*, 'before', T, NM, IP, ONE_D%M_DOT_G_PP_ACTUAL(3), ONE_D%M_DOT_G_PP_ACTUAL(4)
   IF (CALL_HT_1D) THEN
   ...
   ENDIF
!print*, 'after', T, NM, IP, ONE_D%M_DOT_G_PP_ACTUAL(3), ONE_D%M_DOT_G_PP_ACTUAL(4)
   ...
   DO NS=1,N_TRACKED_SPECIES
	...
	!  print*, 'info1', T, NM, IP, NS, MW_RATIO, M_DOT_SINGLE, LP%M_DOT
!	print*, 'info2', T, NM, IP, NS, IIG, JJG, KKG, M_DOT_PPP(IIG,JJG,KKG,NS)
!	print*, 'water vapor', T, NM, IP, NS, IIG, JJG, KKG, M_DOT_PPP(IIG,JJG,KKG,4)
   ENDDO
   ...
   ENDIF
! print*, NM, M_DOT(2,NM), M_DOT(4,NM)
   ...
   IF (SPECIES_MIXTURE(I_WATER)%EVAPORATING) THEN
      ...
      DO N=1,N_LAGRANGIAN_CLASSES
         ...
         IF (LPC%Z_INDEX==I_WATER) MPUA_SUM = MPUA_SUM + ONE_D%LP_MPUA(LPC%ARRAY_INDEX)
			!print*, WALL_INDEX, N, MPUA_SUM, LPC%Z_INDEX, ONE_D%LP_MPUA(LPC%ARRAY_INDEX)
      ENDDO
...
!print*, 'four'
   ...
   ENDIF
!print*, 'Q_WATER_F', T, NM, Q_WATER_F
...
!print*,'six'
   ...
   V_SURF=LP%V
   W_SURF=LP%W
	!print*, T, NM, SIZE(U_SURF,DIM=1), ONE_D%IIG, ONE_D%JJG, ONE_D%KKG, PARTICLE_INDEX, TMP(ONE_D%IIG,ONE_D%JJG,ONE_D%KKG), U(ONE_D%IIG,ONE_D%JJG,ONE_D%KKG), V(ONE_D%IIG,ONE_D%JJG,ONE_D%KKG), W(ONE_D%IIG,ONE_D%JJG,ONE_D%KKG)
...
!print*, T, NM, ONE_D%IIG, ONE_D%JJG, ONE_D%KKG,  ONE_D%TMP_G, ONE_D%TMP_F, LP%U, LP%V, LP%W
...
SELECT CASE(BACKING)
   ...
   CASE(EXPOSED)  ! The backside is exposed to gas in current or adjacent mesh.
      ...
      ENDIF
      Q_CON_B = HTCB*DTMP
!print*, 'Q_WATER_B', T, NM, Q_WATER_B, -SUM(ONE_D_BACK%LP_CPUA(:))
END SELECT
...
!		 print*, 'N', NWP, N
      ...
      IF (.NOT. SF%MT1D) THEN
         ...
         DO NS=1,N_TRACKED_SPECIES
	!print*, 'Before', T,NM, I, NS, M_DOT_G_PP_ADJUST(NS)
	...
	!print*, 'After', T,NM, I, NS, M_DOT_G_PP_ADJUST(NS)
         ENDDO
         DO N=1,SF%N_MATL
...
!print*, 'before Q_S(1)', T, NM, Q_S(1), ONE_D%M_DOT_G_PP_ADJUST(REACTION(1)%FUEL_SMIX_INDEX), SF%H_V, DX_S(1)
   Q_S(1) = Q_S(1) - ONE_D%M_DOT_G_PP_ADJUST(REACTION(1)%FUEL_SMIX_INDEX)*SF%H_V/DX_S(1)
!print*, 'T, NM, Q_S', Q_S(1)
!print*,  'Other', ONE_D%M_DOT_G_PP_ADJUST(REACTION(1)%FUEL_SMIX_INDEX), SF%H_V, DX_S(1)
...
!print*, 'Q_WATER_B1', T,NM, Q_WATER_B, Q_WATER_F
...
!print*, 'CCS', T, NM, NWP, ONE_D%IIG, ONE_D%JJG, ONE_D%KKG, CCS
...
!print*, 'boil', T, NM, NS, NWP, ONE_D%M_DOT_G_PP_ADJUST(NS)   
...
!print*,'sixteen'
	  ...
	  !print*, IIG, JJG, KKG, REACTION_RATE, J, ML%A(J), RHO_S(N), RHO_S0, ML%N_S(J), ML%E(J), TMP_S
            ! power term
