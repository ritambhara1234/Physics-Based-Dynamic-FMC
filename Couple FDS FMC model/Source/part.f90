...
OVERALL_INSERT_LOOP: DO
   ...
   CALL INSERT_VOLUMETRIC_PARTICLES(T,NM)
...
IF (PARTICLE_CFL) THEN
   DO IP=1,NLP
	  ...
	  !print*, T, NM, IP, LP%X, LP%Y, LP%Z, XI,YJ,ZK,II,JJ,KK
   ENDDO
ENDIF
...
SPRINKLER_INSERT_LOOP: DO KS=1,N_DEVC
   ...
   PARTICLE_INSERT_LOOP: DO I=1,N_INSERT
      ...
      LP_INDEX_LOOKUP(I) = NLP
		!print*, 'PARTICLE_TAG', PARTICLE_TAG, NMESHES
...
SUBROUTINE INSERT_VOLUMETRIC_PARTICLES(T,NM)
...
VOLUME_INSERT_LOOP: DO IB=1,N_INIT
   ...
   TOTAL_OR_PER_CELL: IF (IN%N_PARTICLES > 0) THEN
      ...
      INSERT_PARTICLE_LOOP: DO IP=1,N_PARTICLES_INSERT
		...
		!	print*, 'LP_INDEX_LOOKUP(N_INSERT-1)', T, NM, LP_INDEX_LOOKUP(N_INSERT-1)
         ELSE
            NLP = NLP+1
		!	print*, 'NLP1', T, NM, NLP
         ENDIF
         ...
         DO JJ=J1,J2
            II_LOOP: DO II=I1,I2
               ...
               INSERT_PARTICLE_LOOP_2: DO IP = 1, IN%N_PARTICLES_PER_CELL
                  ...
                  ENDIF
					!print*, 'KK, JJ, II, IP', KK, JJ, II, IP
                  IF (NLP+1>MAXIMUM_PARTICLES) THEN
                     CALL REMOVE_OLDEST_PARTICLE(NM,ILPC,NLP,NEW_LP_INDEX)
                     IF (N_INSERT>1) LP_INDEX_LOOKUP(N_INSERT-1) = NEW_LP_INDEX
				!	 print*, 'LP_INDEX_LOOKUP(N_INSERT-1)2', T, NM, N_INSERT, LP_INDEX_LOOKUP(N_INSERT-1)
                  ELSE
                     NLP = NLP+1		! use this NLP in moisture code
				!!!!	print*, T, NM, II, JJ, KK, NLP
                  ENDIF
                  LP_INDEX_LOOKUP(N_INSERT) = NLP
					!	print*, 'NLP3', T, NM, NLP
                  PARTICLE_TAG = PARTICLE_TAG + NMESHES
					!	print*, 'PARTICLE_TAG', T, NM, PARTICLE_TAG
                  CALL ALLOCATE_STORAGE(NM,LAGRANGIAN_PARTICLE_CLASS(ILPC)%SURF_INDEX,LPC_INDEX=ILPC,LP_INDEX=NLP,&
                                        TAG=PARTICLE_TAG,NEW_TAG=.TRUE.)
                  LAGRANGIAN_PARTICLE => MESHES(NM)%LAGRANGIAN_PARTICLE
                  LP=>MESHES(NM)%LAGRANGIAN_PARTICLE(NLP)
...
!		print*, 'WALL(IW)%ONE_D%LP_CPUA(LPC%ARRAY_INDEX)', WALL(IW)%ONE_D%LP_CPUA(LPC%ARRAY_INDEX)
      ENDDO
      DO ICF = 1,N_CFACE_CELLS
...
!		print*, 'CFACE(ICF)%ONE_D%LP_MPUA(LPC%ARRAY_INDEX)', CFACE(ICF)%ONE_D%LP_MPUA(LPC%ARRAY_INDEX)
 ENDDO
   ENDIF
ENDDO
...
!	  print*, 'film thickness', ONE_D%WORK2, T, NM, NLP
...
!			print*, 'TMP_G, Y_GAS', T, NM, TMP_G, Y_GAS
...
!			print*, 'T_BOIL_EFF', T, NM, T_BOIL_EFF
            IF (H_V < 0._EB) THEN
...
!			   print*, 'LP%M_DOT', T, NM, LP%M_DOT
			!print*, 'data2', T, NM, II, JJ, KK, Z_INDEX, M_DOT_PPP(II,JJ,KK,Z_INDEX)
...
!			   print*, 'MVAP_TOT', T, NM, II,JJ, KK, MVAP_TOT(II,JJ,KK)
!			print*, 'd2', 'M_VAP, M_DOT(4,NM)', M_VAP, M_DOT(4,NM)
...
!			print*, M_VAP
               ...
               ELSE EVAP_ALL
                  ...
                  Q_CON_WALL = Q_CON_WALL*Q_FRAC
                  TMP_DROP_NEW = T_BOIL_EFF
!				  print*, 'd4', M_VAP, Q_FRAC, H_V, Q_TOT
               ENDIF EVAP_ALL
...
!				print*, 'M_GAS_NEW', T, NM, II, JJ, KK, M_GAS_NEW, RVC
               ! Compute contribution to the divergence
               ...
               M_DOT_PPP(II,JJ,KK,Z_INDEX) = M_DOT_PPP(II,JJ,KK,Z_INDEX) + M_VAP*RVC*WGT/DT
               LP%M_DOT = LP%M_DOT + M_VAP/DT
!				print*, II,JJ,KK,Z_INDEX,LP%M_DOT
               ...
               MVAP_TOT(II,JJ,KK) = MVAP_TOT(II,JJ,KK) + WGT*M_VAP
!				print*, 'MVAP_TOT(II,JJ,KK)', T, NM, II, JJ, KK, MVAP_TOT(II,JJ,KK)
