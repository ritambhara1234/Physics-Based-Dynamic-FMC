...
IF (SOLID_PARTICLES) THEN
   ...
   ! end if
      ...
      IF (LPC%SOLID_PARTICLE .OR. LPC%MASSLESS_TARGET) THEN  ! Target particles are included to get gas phase values
         ...
         IF (SF%THERMAL_BC_INDEX==THERMALLY_THICK .AND. CALL_HT_1D) CALL SOLID_HEAT_TRANSFER_1D_LP(NM,T,DT_BC,PARTICLE_INDEX=IP)
		 
		 IF (NM == 2) THEN
		 ! print*, T, NM, LP%ONE_D%TMP_F
		 ENDIF
      ENDIF
   ENDDO
ENDIF
   ...
   K_LOOP_2: DO K=OB%K1+1,OB%K2
      J_LOOP_2: DO J=OB%J1+1,OB%J2
         I_LOOP_2: DO I=OB%I1+1,OB%I2
            ...
            CALL PYROLYSIS(NM,MS%N_MATL,MS%MATL_INDEX,OB%MATL_SURF_INDEX,IIG,JJG,KKG,TMP_S,TMP_F,IOR,&
...
REAL(EB), POINTER, DIMENSION(:,:,:,:) :: ZZP=>NULL()
REAL(EB) :: H_S_B
!REAL(EB), POINTER, DIMENSION(:,:,:) :: TEMP_SOLID
! REAL(EB), POINTER, DIMENSION(:) :: TEMP_PRINT
! INTEGER :: data_tot, I_IN, J_IN
! INTEGER, allocatable, dimension(:,:) ::I_CALC
...
! data_tot = IIG*JJG
!allocate(TEMP_SOLID(IIG,JJG,KKG));
! allocate(TEMP_PRINT(data_tot));
! allocate(I_CALC(IIG,JJG));
   ...
   DO NS=1,N_TRACKED_SPECIES
	  ...
	  !print*, 'TEMP_SURF(IIG,JJG,KKG,1)', TEMP_SURF(IIG,JJG,KKG,1)
   ENDDO

	  ! ! Surface temperature
	  ! TEMP_SOLID(IIG,JJG,1) = LP%ONE_D%TMP_F
	  ! M%TEMP_SURF(IIG,JJG,1,2) = TEMP_SOLID(IIG,JJG,1)
	  
!	  print*, T, NM, IIG, JJG, KKG, LP%ONE_D%TMP_F

   ! IF (INT(T) == 5) THEN
  		! print*, 'LP%ONE_D%TMP_F', T, NM, IIG, JJG, KKG, IP, LP%ONE_D%TMP_F
   ! ENDIF
   
   
   ! do J_IN = 1,JJG
		! do I_IN = 1,IIG
			! I_CALC(I_IN,J_IN) = (J_IN-1)*IIG + I_IN
			! TEMP_PRINT(I_CALC(I_IN,J_IN)) = LP%ONE_D%TEMP_F
		! end do
   ! end do
   
   ! IF (INT(T) == 5) THEN
  		! print*, 'TEMP_PRINT(I_CALC(I_IN,J_IN))', T, NM, IIG, JJG, KKG, TEMP_PRINT(I_CALC(I_IN,J_IN))
   ! ENDIF
   ...
   ENDIF CALC_LP_MASS
   ! IF (INT(T) == 5) THEN
	! ! IF (IP .LE. (IIG*JJG)) THEN
  		! print*, 'LP%ONE_D%TMP_F', T, NM, IIG, JJG, KKG, IP, ONE_D%TMP_F
	! ! ENDIF
   ! ENDIF
ENDDO PARTICLE_LOOP
...
ELSEIF (PRESENT(PARTICLE_INDEX)) THEN UNPACK_WALL_PARTICLE
   ...
   V_SURF=LP%V
   W_SURF=LP%W
!print*, 'T,NM,Q_WATER_F', T,NM,Q_WATER_F
...
!		print*, 'LAYER_INDEX(I)', LAYER_INDEX(I)
      CALL PYROLYSIS(NM,SF%N_MATL,SF%MATL_INDEX,SURF_INDEX,ONE_D%IIG,ONE_D%JJG,ONE_D%KKG,ONE_D%TMP(I),ONE_D%TMP_F,ONE_D%IOR,&
                     ...
                     R_DROP=R_SURF,LPU=U_SURF,LPV=V_SURF,LPW=W_SURF,Time=T)
...
! Modifications are made to this subroutine to account for the addition of moisture model
! Works only for moisture model with Lagrangian particles
! Make sure to use it only when the Lagrangian particle occupy the entire part that runs in a processor
...
SUBROUTINE SOLID_HEAT_TRANSFER_1D_LP(NM,T,DT_BC,PARTICLE_INDEX,WALL_INDEX,CFACE_INDEX)

USE GEOMETRY_FUNCTIONS
USE MATH_FUNCTIONS, ONLY: EVALUATE_RAMP, INTERPOLATE1D_UNIFORM
USE COMP_FUNCTIONS, ONLY: SHUTDOWN
USE PHYSICAL_FUNCTIONS, ONLY: GET_SPECIFIC_GAS_CONSTANT,GET_SENSIBLE_ENTHALPY
USE GLOBAL_CONSTANTS, ONLY:N_layers
REAL(EB), INTENT(IN) :: DT_BC,T
INTEGER, INTENT(IN) :: NM
INTEGER, INTENT(IN), OPTIONAL:: WALL_INDEX,PARTICLE_INDEX,CFACE_INDEX
REAL(EB) :: DUMMY,DTMP,QDXKF,QDXKB,RR,RFACF,RFACB,RFACF2,RFACB2, &
...
REAL(EB), POINTER, DIMENSION(:) :: DELTA_TMP
INTEGER :: IIB,JJB,KKB,IWB,NWP,KK,I,NR,NL,N,I_OBST,NS,N_LAYER_CELLS_NEW(MAX_LAYERS),N_CELLS,EXPON,ITMP,ITER, INSIDE_J, INSIDE_I
REAL(EB) :: SMALLEST_CELL_SIZE(MAX_LAYERS),THICKNESS
REAL(EB),ALLOCATABLE,DIMENSION(:,:) :: INT_WGT
REAL(EB), DIMENSION(:) :: RHO_ZZ_F(1:N_TRACKED_SPECIES),RHO_ZZ_B(1:N_TRACKED_SPECIES),ZZ_GET(1:N_TRACKED_SPECIES)
INTEGER  :: NWP_NEW,I_GRAD,IZERO,SURF_INDEX,SURF_INDEX_BACK,BACKING
LOGICAL :: REMESH,E_FOUND,CHANGE_THICKNESS,CONST_C(NWP_MAX),BOIL_CYCLE
CHARACTER(MESSAGE_LENGTH) :: MESSAGE
INTEGER :: data_num, int_mois, num_layers
INTEGER, allocatable, dimension(:,:) :: ki
TYPE (MESH_TYPE), POINTER :: M
REAL(EB):: MOIS, RHO_DRY_LPC, RHO_S0_MOIS
CALL POINT_TO_MESH(NM)
M => MESHES(NM)

allocate (ki(IBAR,JBAR))
...
ELSEIF (PRESENT(CFACE_INDEX)) THEN UNPACK_WALL_PARTICLE
   ...
   BACKING = SF%BACKING
   IF (CFA%BACK_INDEX==0 .AND. SF%BACKING==EXPOSED) BACKING = VOID
!print*, 'T,NM,Q_WATER_F,CFACE_INDEX', T,NM,CFACE_INDEX,Q_WATER_F
ELSEIF (PRESENT(PARTICLE_INDEX)) THEN UNPACK_WALL_PARTICLE
...
! IF (INT(T) == 1 .AND. NM == 2) then
	! print*, M_DOT_G_PP_ADJUST(1:N_TRACKED_SPECIES)
! ENDIF
...
!print*, DTMP
! Compute back side emissivity
! print*, T, NM, PARTICLE_INDEX, ONE_D%TMP_F
E_WALLB = SF%EMISSIVITY_BACK
IF (E_WALLB < 0._EB .AND. BACKING /= INSULATED) THEN
   E_WALLB = 0._EB
   VOLSUM  = 0._EB
...
!print*, 'five', 5
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!Changing density as per the moisture model
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!print*, 'four', 4

		!IF (NM .GE. 6 .AND. NM .LE. 7) THEN
		
			data_num = PARTICLE_INDEX

			MOIS = 0;
				
			do int_mois = 1, (num_layers)
				MOIS = MOIS + (M%koba_m_fin(data_num,int_mois));		
			end do
			
			! IF (MOIS .LE. 0) THEN
				! MOIS = 0;
			! ELSE
				! MOIS = MOIS/num_layers;
			! ENDIF
			
			MOIS = MOIS/(num_layers);
			
			RHO_DRY_LPC = 593.0
			RHO_S0_MOIS = 1/(((1-MOIS)/RHO_DRY_LPC)+(MOIS/1000));
			RHO_S0 = RHO_S0_MOIS
			
			! IF (NM == 13) THEN
				! print*, NM, T, DT_BC, PARTICLE_INDEX, MOIS, RHO_S0
			! ENDIF
			!print*, RHO_S0
			!print*, 'NM,T,RHO_S0',NM,T,RHO_S0
		
		!ELSEIF (NM .GE. 10 .AND. NM .LE. 11) THEN
...
!	  print*, 'RHO_S0, SF%LAYER_DENSITY(LAYER_INDEX(I))', RHO_S0, SF%LAYER_DENSITY(LAYER_INDEX(I))
      CALL PYROLYSIS(NM,SF%N_MATL,SF%MATL_INDEX,SURF_INDEX,ONE_D%IIG,ONE_D%JJG,ONE_D%KKG,ONE_D%TMP(I),ONE_D%TMP_F,ONE_D%IOR,&
                     RHO_DOT_TEMP(1:SF%N_MATL),RHO_TEMP(1:SF%N_MATL),RHO_S0,ONE_D%X(I-1),DT_BC-T_BC_SUB,&
                     M_DOT_G_PPP_ADJUST,M_DOT_G_PPP_ACTUAL,M_DOT_S_PPP,Q_S(I),Q_DOT_G_PPP,Q_DOT_O2_PPP,T_BOIL_EFF,&
                     R_DROP=R_SURF,LPU=U_SURF,LPV=V_SURF,LPW=W_SURF,Time=T)
      ...
      IF (.NOT. SF%MT1D) THEN
		...
		! IF (NM .GE. 2 .AND. NM .LE. 6) THEN
			! print*, 'N, M_DOT_S_PP, GEOM_FACTOR', T, NM, N, M_DOT_S_PP(N), GEOM_FACTOR
		! ENDIF
...
ELSEIF (SF%PYROLYSIS_MODEL==PYROLYSIS_SPECIFIED) THEN PYROLYSIS_PREDICTED_IF
   ...
   Q_S(1) = Q_S(1) - ONE_D%M_DOT_G_PP_ADJUST(REACTION(1)%FUEL_SMIX_INDEX)*SF%H_V/DX_S(1)
!print*, 'Q_S(1)', Q_S(1)
!print*, REACTION(1)%FUEL_SMIX_INDEX
...
IF (SF%SPECIFIED_HEAT_SOURCE) THEN
   ...
   ENDDO
ENDIF
!print*, 'Specified heat source Q_S', Q_S(1), Q_S(NWP)
! Add special convection term for Boundary Fuel Model
...
!   print*, 'Boundary fuel Q_S', Q_S(1), Q_S(NWP)
...
IF ( .NOT.RADIATION .OR. SF%INTERNAL_RADIATION ) THEN
   QDXKF = (HTCF*ONE_D%TMP_G + Q_WATER_F)/(KODXF+RFACF)
   QDXKB = (HTCB*TMP_BACK    + Q_WATER_B)/(KODXB+RFACB)
!   print*, 'Q_WATER_F', Q_WATER_F   
ELSE
   QDXKF = (HTCF*ONE_D%TMP_G + Q_WATER_F + ONE_D%Q_RAD_IN + 3._EB*ONE_D%EMISSIVITY*SIGMA*ONE_D%TMP_F**4) / (KODXF+RFACF)
   QDXKB = (HTCB*TMP_BACK    + Q_WATER_B + Q_RAD_IN_B     + 3._EB*E_WALLB*SIGMA*ONE_D%TMP_B**4         ) / (KODXB+RFACB)
!      print*, 'ELSE Q_WATER_F', Q_WATER_F, ONE_D%Q_RAD_IN, Q_RAD_IN_B 
ENDIF
CCS(1)   = CCS(1)   - BBS(1)  *QDXKF
CCS(NWP) = CCS(NWP) - AAS(NWP)*QDXKB
DDT(1:NWP) = DDS(1:NWP)
...
do INSIDE_J = 1,JBAR
	do INSIDE_I = 1,IBAR
		ki(INSIDE_I,INSIDE_J) = (INSIDE_J-1)*IBAR + INSIDE_I
	end do
end do

! DO I = 1,NWP
	! M%koba_tl_fin(I,5) = ONE_D%Q_RAD_IN 
! END DO

! DO I = 1,NWP
	! print*, NM, T, PARTICLE_INDEX, ONE_D%Q_RAD_IN , Q_RAD_IN_B, Q_S(I)
! ENDDO
...
MASS_TRANSFER_1D: IF (SF%MT1D) THEN
   ...
   DO NS = 1,SF%N_SPEC
      ...
      DO I=1,NWP
         ...
         ONE_D%SPEC_COMP(NS)%RHO_ZZ(I) = MAX(0._EB,ONE_D%SPEC_COMP(NS)%RHO_ZZ(I)) ! guarantee boundedness
      ENDDO
!	  print*, 'ONE_D%SPEC_COMP(NS)%RHO_ZZ(I)', ONE_D%SPEC_COMP(NS)%RHO_ZZ(I)
!      IF (NS==2) write(102,*) T,ONE_D%SPEC_COMP(NS)%RHO_ZZ(1:NWP)
   ENDDO
ENDIF MASS_TRANSFER_1D
...
END SUBROUTINE SOLID_HEAT_TRANSFER_1D_LP
...
SUBROUTINE PYROLYSIS(NM,N_MATS,MATL_INDEX,SURF_INDEX,IIG,JJG,KKG,TMP_S,TMP_F,IOR,RHO_DOT_OUT,RHO_S,RHO_S0,DEPTH,DT_BC,&
                     M_DOT_G_PPP_ADJUST,M_DOT_G_PPP_ACTUAL,M_DOT_S_PPP,Q_DOT_S_PPP,Q_DOT_G_PPP,Q_DOT_O2_PPP,T_BOIL_EFF,&
                     Time,R_DROP,LPU,LPV,LPW)
...
REAL(EB), INTENT(IN), OPTIONAL :: Time
   ...
   REACTION_LOOP: DO J=1,ML%N_REACTIONS  ! Tech Guide: Sum over the reactions, beta
      ...
      SELECT CASE (ML%PYROLYSIS_MODEL)
         ...
         CASE (PYROLYSIS_LIQUID)
			print*, 'PYROLYSIS_LIQUID, J,IIG,JJG,KKG', J,N,IIG,JJG,KKG
         ...
         CASE (PYROLYSIS_SOLID)
			...
			! IF (NM .GE. 2 .AND. NM .LE. 6) THEN
				! print*, 'REACTION_RATE, DTMP, TMP_S',REACTION_RATE, DTMP, TMP_S
			! ENDIF
			...
			! IF (NM .GE. 2 .AND. NM .LE. 6) THEN
				! print*, 'REACTION_RATE, ML%N_T(J)', REACTION_RATE, ML%N_T(J)
			! ENDIF
		...
		print*, 'PC, ML%H_R_I(J)', ML%H_R_I(J)
               ELSE
                  H_R = ML%H_R(J)
		print*, 'PC, ML%H_R(J)', J, ML%H_R(J)
               ENDIF
               REACTION_RATE = REACTION_RATE / ((ABS(H_R)/1000._EB) * DT_BC)		  
            ENDIF

			 ! IF (NM .GE. 2 .AND. NM .LE. 6) THEN
				! print*, 'ML%PCR(J), H_R, REACTION_RATE', ML%N_REACTIONS, ML%PCR(J), H_R, REACTION_RATE
			 ! ENDIF
			...
			! IF (NM .GE. 2 .AND. NM .LE. 6) THEN
				! print*, 'REACTION_RATE, ZZ_GET(1:N_TRACKED_SPECIES), X_O2', REACTION_RATE, ZZ_GET(1:N_TRACKED_SPECIES), X_O2
			! ENDIF
			
            RHO_DOT = MIN(RHO_S0*REACTION_RATE,RHO_S(N)/DT_BC)  ! Tech Guide: rho_s(0)*r_alpha,beta kg/m3/s

			! IF (NM .GE. 2 .AND. NM .LE. 6) THEN
				! print*, 'RHO_DOT, RHO_S0, REACTION_RATE, RHO_S(N), DT_BC', RHO_DOT, RHO_S0, REACTION_RATE, RHO_S(N), DT_BC
			! ENDIF
!		print*, RHO_DOT, N
	  ...
	  !print*, 'RHO_DOT_OUT(N), N', RHO_DOT_OUT(N), N
      ...
      ! Calculate various energy and mass source terms
		 ...
		 ! IF (NM .GE. 2 .AND. NM .LE. 6) THEN
			! print*, 'BEFORE, N, Q_DOT_S_PPP, M_DOT_S_PPP(N), RHO_DOT, H_R', N, Q_DOT_S_PPP, M_DOT_S_PPP(N), RHO_DOT, H_R
		 ! ENDIF
      ...
      M_DOT_S_PPP(N) = M_DOT_S_PPP(N) - RHO_DOT   ! m_dot_alpha''' = -rho_s(0) * sum_beta r_alpha,beta
		 ...
		 ! IF (NM .GE. 2 .AND. NM .LE. 6) THEN
			! print*, 'AFTER, N, Q_DOT_S_PPP, M_DOT_S_PPP(N), RHO_DOT, H_R', N, Q_DOT_S_PPP, M_DOT_S_PPP(N), RHO_DOT, H_R
		 ! ENDIF
