...
!		print*, 'four', IIG,JJG,ROS_HEAD(IIG,JJG)
   ENDDO
ENDDO
...
SELECT CASE(ROTHERMEL_FUEL_INDEX)
    CASE(1)  ! 'Short Grass'
       w0d1=1.0     ; w0d2=0.        ; w0d3=0.        ; w0lh=0.        ; w0lw=0.     ! dry mass per unit area (kg/m2)
       svd1=9770.     ; svd2=358.      ; svd3=98.       ; svlh=4921.     ; svlw=4921.  ! surface area to volume (1/m)
...
!print*, 'inside level set', SURF_INDEX, REACTION(1)%FUEL_SMIX_INDEX, SF%MASS_FLUX(REACTION(1)%FUEL_SMIX_INDEX), SF%BURN_DURATION
ENDIF
