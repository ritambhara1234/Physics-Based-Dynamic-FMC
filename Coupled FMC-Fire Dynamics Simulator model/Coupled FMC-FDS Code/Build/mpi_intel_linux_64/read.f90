   ...
   IF (SF%THERMAL_BC_INDEX==THERMALLY_THICK) THEN
      ...
      DO NN=1,SF%N_CELLS_INI
         SELECT CASE (SF%GEOMETRY)
            CASE (SURF_CARTESIAN)
!			   print*, 'read cartesian before', SF%RHO_0(NN,1:SF%N_MATL), NN, N, MASS
               MASS = MASS + (SF%X_S(NN)-SF%X_S(NN-1))*SUM(SF%RHO_0(NN,1:SF%N_MATL))
               VOLUME = VOLUME + (SF%X_S(NN)-SF%X_S(NN-1))
!			   print*, 'read cartesian after', SF%RHO_0(NN,1:SF%N_MATL), NN, N, MASS
            CASE (SURF_CYLINDRICAL)
!			print*, 'read cylindrical before', SF%RHO_0(NN,1:SF%N_MATL), NN, N, MASS
...
!			   print*, 'read cylindrical after', SF%RHO_0(NN,1:SF%N_MATL), NN, N, MASS
            CASE (SURF_SPHERICAL)
!			print*, 'read spherical before', SF%RHO_0(NN,1:SF%N_MATL), NN, N, MASS
               R_I = SF%INNER_RADIUS + SF%THICKNESS - SF%X_S(NN)
               R_O = SF%INNER_RADIUS + SF%THICKNESS - SF%X_S(NN-1)
               MASS   = MASS   + (R_O**3-R_I**3)*SUM(SF%RHO_0(NN,1:SF%N_MATL))
               VOLUME = VOLUME + (R_O**3-R_I**3)
!			   print*, 'read spherical after', SF%RHO_0(NN,1:SF%N_MATL), NN, N, MASS
         END SELECT
      ENDDO
...
!			print*, 'NR, PEAK_REACTION_RATE,ML%RATE_REF(NR)', NR, PEAK_REACTION_RATE, ML%RATE_REF(NR)
         ELSE
            PEAK_REACTION_RATE = 2._EB*ML%HEATING_RATE(NR)*(1._EB-SUM(NU_MATL(:,NR)))/ML%PYROLYSIS_RANGE(NR)
!			print*, 'NR, PEAK_REACTION_RATE', NR, PEAK_REACTION_RATE
         ENDIF
...
!			print*, 'SUM_D, MATL_MASS_FRACTION(NL,NN), MATERIAL(NNN)%RHO_S', SUM_D, MATL_MASS_FRACTION(NL,NN), MATERIAL(NNN)%RHO_S
         ENDDO
         RHO_DRY = 1._EB/SUM_D
	...
	!		print*, 'NL, NN, X_AIR,RHO_DRY,MATL_MASS_FRACTION(NL,NN) before', NL, NN, X_AIR,RHO_DRY,MATL_MASS_FRACTION(NL,NN), MATL_MASS_FRACTION(NL,1:NN-1)
    !        MATL_MASS_FRACTION(NL,1:NN-1) = MATL_MASS_FRACTION(NL,1:NN-1)*(1._EB-MATL_MASS_FRACTION(NL,NN))
	!		print*, 'NL, NN, MATL_MASS_FRACTION(NL,1:NN-1)', NL, NN, MATL_MASS_FRACTION(NL,1:NN-1)
            NN = NN+1
         ENDIF
...
!			print*, 'MATL_MASS_FRACTION(NL,NN), MOISTURE_FRACTION(NL) before', MATL_MASS_FRACTION(NL,NN), MOISTURE_FRACTION(NL)
   ...
   COUNT_LAYERS: DO NL=1,MAX_LAYERS
	...
	!	 print*, 'NL, NN, SF%LAYER_MATL_FRAC(NL,NN)', NL, NN, SF%LAYER_MATL_NAME(NL,NN), SF%LAYER_MATL_FRAC(NL,NN)
         DO NNN=1,N_MATL
            IF (MATL_NAME(NNN)==NAME_LIST(N_LIST)) THEN
               INDEX_LIST(N_LIST) = NNN
               SF%LAYER_MATL_INDEX(NL,NN) = NNN
		!	   print*, 'material index', SF%LAYER_MATL_INDEX(NL,NN), NNN
               SF%LAYER_DENSITY(NL) = SF%LAYER_DENSITY(NL)+SF%LAYER_MATL_FRAC(NL,NN)/MATERIAL(NNN)%RHO_S
		!	   print*, 'NL,SF%LAYER_DENSITY(NL),SF%LAYER_MATL_FRAC(NL,NN),MATERIAL(NNN)%RHO_S,SF%LAYER_DENSITY(NL)', NL, NNN, SF%LAYER_DENSITY(NL), SF%LAYER_MATL_FRAC(NL,NN), MATERIAL(NNN)%RHO_S,SF%LAYER_DENSITY(NL)
	...
	!  print*, 'NL, SF%LAYER_DENSITY(NL)', NL, SF%LAYER_DENSITY(NL)
...
!	  	  print*, 'SF%THICKNESS, SF%LAYER_THICKNESS(NL)', SF%THICKNESS, SF%LAYER_THICKNESS(NL)
   ENDDO COUNT_LAYERS
...
!	print*, 'N_CELLS_MAX, SF%N_LAYER_CELLS(NL)', N_CELLS_MAX, SF%N_LAYER_CELLS(NL)
...
!	print*, 'SF%N_CELLS_INI, SF%N_LAYER_CELLS(NL)', SF%N_CELLS_INI, SF%N_LAYER_CELLS(NL)
   ENDDO LAYER_LOOP
...
!		print*, 'SURFACE(LPC%SURF_INDEX), SURFACE(LPC%SURF_INDEX)%MOISTURE_FRACTION(1)', LPC%SURF_INDEX, SURFACE(LPC%SURF_INDEX)%MOISTURE_FRACTION(1)
