...
!print*, 'NNPC loop cycled', SF%NPPC
...
!print*, 'IN%N_PARTICLES', IN%N_PARTICLES
...
!print*, 'N_PARTICLES_INSERT', N_PARTICLES_INSERT
         ...
         N_INSERT = N_INSERT + 1
		!print*, 'T,NM,N_INSERT', T,NM,N_INSERT
...
IF (IN%ID/='null') THEN
!print*, 'Works for Mell case DEVC and or PROF'
   DO ND=1,N_DEVC
      DV => DEVICE(ND)
...
!print*, 'Special over ride works for Mell case'
...
IF (IN%ID/='null') THEN
!	print*, 'Ad Su Te works for Mell case'
   DO ND=1,N_DEVC
      DV => DEVICE(ND)
...
!print*, 'MASS_SUM, LP%PWT, LP%MASS', MASS_SUM, LP%PWT, LP%MASS 
...
!	print*, 'LPC%SOLID_PARTICLE works'
...
!			print*, 'calculates SCALE_FACTOR', SCALE_FACTOR
         ELSE
            SCALE_FACTOR = 1._EB
         ENDIF
...
!				    print*, 'SF%THICKNESS, SF%LENGTH', SF%THICKNESS, SF%LENGTH
                  X1 = SUM(SF%LAYER_THICKNESS)*SCALE_FACTOR
!				  			 print*, 'X1', X1	
                  DO N=SF%N_LAYERS,1,-1
!				    print*, 'SF%N_LAYERS', SF%N_LAYERS
                     X2 = X1 - SF%LAYER_THICKNESS(N)*SCALE_FACTOR
!					 					 print*, 'X2', X2
                     LP%MASS = LP%MASS + SF%LENGTH*SF%LAYER_DENSITY(N)*PI*(X1**2-X2**2)
!					 print*, 'LP%MASS, SF%LAYER_DENSITY(N)', LP%MASS, SF%LAYER_DENSITY(N)
                     X1 = X2
                  ENDDO
               CASE (SURF_SPHERICAL)
...
!   print*, 'works ONE_D%T_IGN, SF%FIRE_SPREAD_RATE', ONE_D%T_IGN, SF%FIRE_SPREAD_RATE
ELSE
   ONE_D%T_IGN = SF%T_IGN
!   print*, 'works SF%T_IGN', SF%T_IGN
ENDIF
...
!print*, 'WAKE REDUCTION C_DRAG', C_DRAG
