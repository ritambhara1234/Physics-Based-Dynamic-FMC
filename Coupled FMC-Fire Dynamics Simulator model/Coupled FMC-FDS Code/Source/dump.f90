...
TYPE (MOIS_SLICE_TYPE), POINTER :: SLM=>NULL()
...
PUBLIC ASSIGN_FILE_NAMES,INITIALIZE_GLOBAL_DUMPS,INITIALIZE_MESH_DUMPS,WRITE_STATUS_FILES, &
       ...
       DUMP_UVW,DUMP_GEOM,UPDATE_DEVICES_2, DUMP_TEMP, PROFILE_AVERAGE
...
INTEGER :: NM,I,N,NFM, INP_N_LAYERS
...
ALLOCATE(FN_PROFAVG(NMESHES))
ALLOCATE(LU_PROFAVG(NMESHES))
ALLOCATE(FN_HPROFAVG(NMESHES))
ALLOCATE(LU_HPROFAVG(NMESHES))

ALLOCATE(FN_MOISTURE(NMESHES,INP_N_LAYERS))
ALLOCATE(LU_MOISTURE(NMESHES,INP_N_LAYERS))
ALLOCATE(FN_SPEC_HUM(NMESHES,INP_N_LAYERS))
ALLOCATE(LU_SPEC_HUM(NMESHES,INP_N_LAYERS))
ALLOCATE(FN_TEMP_A(NMESHES,INP_N_LAYERS))
ALLOCATE(LU_TEMP_A(NMESHES,INP_N_LAYERS))
ALLOCATE(FN_TEMP_M(NMESHES,INP_N_LAYERS))
ALLOCATE(LU_TEMP_M(NMESHES,INP_N_LAYERS))
ALLOCATE(FN_LIQ(NMESHES,INP_N_LAYERS))
ALLOCATE(LU_LIQ(NMESHES,INP_N_LAYERS))
ALLOCATE(FN_TEMP_L(NMESHES,INP_N_LAYERS))
ALLOCATE(LU_TEMP_L(NMESHES,INP_N_LAYERS))
...
ALLOCATE(FN_TEMP(NMESHES))
ALLOCATE(LU_TEMP(NMESHES))
ALLOCATE(FN_VELO(NMESHES))
ALLOCATE(LU_VELO(NMESHES))
ALLOCATE(LU_NUMBER(NMESHES))
ALLOCATE(FN_NUMBER(NMESHES))

ALLOCATE(FN_RTEST(NMESHES))
ALLOCATE(LU_RTEST(NMESHES))
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   LU_RTEST(NM) = GET_FILE_NUMBER()
   WRITE(FN_RTEST(NM),'(A,A,I0,A)') TRIM(RESTART_CHID),'_temp_',NM,'.txt'
!   LU_TEMP(NM)    = GET_FILE_NUMBER()
!   WRITE TRIM(CHID)//'_temp.csv'
!   LU_VELO(NM)    = GET_FILE_NUMBER()
!   WRITE TRIM(CHID)//'_velo.csv'

!  '(A,A,I0,A)'

LU_TEMP(NM) = GET_FILE_NUMBER()
WRITE(FN_TEMP(NM),'(A,A,I0,A)') TRIM(CHID),'_temp_',NM,'.txt'
LU_VELO(NM) = GET_FILE_NUMBER()
WRITE(FN_VELO(NM),'(A,A,I0,A)') TRIM(CHID),'_velo_',NM,'.txt'
LU_NUMBER(NM) = GET_FILE_NUMBER()
WRITE(FN_NUMBER(NM),'(A,A,I0,A)') TRIM(CHID),'_number_',NM,'.txt'


LU_MOISTURE(NM,INP_N_LAYERS) = GET_FILE_NUMBER()
!WRITE(FN_MOISTURE(NM,5),'(A,A,I0,A)') TRIM(CHID),'_Mois_',NM,'.txt'
LU_SPEC_HUM(NM,INP_N_LAYERS) = GET_FILE_NUMBER()
!WRITE(FN_SPEC_HUM(NM,5),'(A,A,I0,A)') TRIM(CHID),'_SpecHumi_',NM,'.txt'
LU_TEMP_A(NM,INP_N_LAYERS) = GET_FILE_NUMBER()
!WRITE(FN_TEMP_A(NM,5),'(A,A,I0,A)') TRIM(CHID),'_AirTemp_',NM,'.txt'
LU_TEMP_M(NM,INP_N_LAYERS) = GET_FILE_NUMBER()
!WRITE(FN_TEMP_M(NM,5),'(A,A,I0,A)') TRIM(CHID),'_LitterTemp_',NM,'.txt'
LU_LIQ(NM,INP_N_LAYERS) = GET_FILE_NUMBER()
!WRITE(FN_LIQ(NM,5),'(A,A,I0,A)') TRIM(CHID),'_Liquid_',NM,'.txt'
LU_TEMP_L(NM,INP_N_LAYERS) = GET_FILE_NUMBER()
!WRITE(FN_TEMP_L(NM,5),'(A,A,I0,A)') TRIM(CHID),'_LiqTemp_',NM,'.txt'



LU_PROFAVG(NM) = GET_FILE_NUMBER()
!WRITE(FN_PROFAVG(NM),'(A,A,I0,A)') TRIM(CHID),'_pavg_',NM,'.txt'

LU_HPROFAVG(NM) = GET_FILE_NUMBER()
WRITE(FN_HPROFAVG(NM),'(A,A,I0,A)') TRIM(CHID),'_hprofavg_',NM,'.txt'
...
!> \brief Dump average data to a file for finding horizontal average
!> \param T Current time (s)
!> \param DT Current time step size (s)
!> \param NM Mesh number
!> \ICYC is the number of iterations

SUBROUTINE PROFILE_AVERAGE(T,DT,NM,ICYC)
...
!REAL(EB), POINTER, DIMENSION(:,:,:) :: TEMP_R, U_VEL_R, V_VEL_R, W_VEL_R
! REAL(EB), ALLOCATABLE, DIMENSION(:,:,:) :: TEMP_R, U_VEL_R, V_VEL_R, W_VEL_R
INTEGER :: NOM,N,ITM


INTEGER :: I,J,K,IT,count_n,Total_NM
INTEGER :: GEOM_INDEX=-1,TRNF_INDEX=-1,INDEX2=0,Z_INDEX=-999,Y_INDEX=-999,MATL_INDEX=0,&
              PART_INDEX=0,VELO_INDEX=0,PIPE_INDEX=0,PROP_INDEX=0,REAC_INDEX=0, data_tot, data_tot1
INTEGER :: num
...
Total_NM = N_MPI_PROCESSES

! ALLOCATE(TEMP_R(IBP1+1,JBP1+1,KBP1+1))
! ALLOCATE(U_VEL_R(IBP1+1,JBP1+1,KBP1+1))
! ALLOCATE(V_VEL_R(IBP1+1,JBP1+1,KBP1+1))
! ALLOCATE(W_VEL_R(IBP1+1,JBP1+1,KBP1+1))
...
! DO k = 0,KBP1
	! DO j = 0,JBP1
		! DO i = 0,IBP1
		! TEMP_R(I,J,K) = TMP(I,J,K)
		! U_VEL_R(I,J,K) = U(I,J,K)
		! V_VEL_R(I,J,K) = V(I,J,K)
		! W_VEL_R(I,J,K) = W(I,J,K)
	   ! ENDDO
    ! ENDDO
! ENDDO
		  ...
		  DO k = 0,KBP1
			  DO j = 0,JBP1
				  DO i = 0,IBP1
					  ! M%AVG_T(I,J,K) = TEMP_R(I,J,K);
					  ! M%AVG_U(I,J,K) = U_VEL_R(I,J,K);
					  ! M%AVG_V(I,J,K) = V_VEL_R(I,J,K);
					  ! M%AVG_W(I,J,K) = W_VEL_R(I,J,K);
					  ! M%AVG_TW(I,J,K) = TEMP_R(I,J,K)*W_VEL_R(I,J,K);
					  ! M%AVG_TU(I,J,K) = TEMP_R(I,J,K)*U_VEL_R(I,J,K);
					  ! M%AVG_TV(I,J,K) = TEMP_R(I,J,K)*V_VEL_R(I,J,K);
					  ! M%AVG_UV(I,J,K) = U_VEL_R(I,J,K)*V_VEL_R(I,J,K);
					  ! M%AVG_WV(I,J,K) = W_VEL_R(I,J,K)*U_VEL_R(I,J,K);
					  ! M%AVG_WU(I,J,K) = W_VEL_R(I,J,K)*V_VEL_R(I,J,K);
					  ! M%AVG_U2(I,J,K) = U_VEL_R(I,J,K)*U_VEL_R(I,J,K);
					  ! M%AVG_V2(I,J,K) = V_VEL_R(I,J,K)*V_VEL_R(I,J,K);
					  ! M%AVG_W2(I,J,K) = W_VEL_R(I,J,K)*W_VEL_R(I,J,K);
					  ! M%AVG_T2(I,J,K) = TEMP_R(I,J,K)*TEMP_R(I,J,K);
					  M%AVG_T(I,J,K) = TMP(I,J,K);
					  M%AVG_U(I,J,K) = U(I,J,K);
					  M%AVG_V(I,J,K) = V(I,J,K);
					  M%AVG_W(I,J,K) = W(I,J,K);
					  M%AVG_TW(I,J,K) = TMP(I,J,K)*W(I,J,K);
					  M%AVG_TU(I,J,K) = TMP(I,J,K)*U(I,J,K);
					  M%AVG_TV(I,J,K) = TMP(I,J,K)*V(I,J,K);
					  M%AVG_UV(I,J,K) = U(I,J,K)*V(I,J,K);
					  M%AVG_WV(I,J,K) = W(I,J,K)*U(I,J,K);
					  M%AVG_WU(I,J,K) = W(I,J,K)*V(I,J,K);
					  M%AVG_U2(I,J,K) = U(I,J,K)*U(I,J,K);
					  M%AVG_V2(I,J,K) = V(I,J,K)*V(I,J,K);
					  M%AVG_W2(I,J,K) = W(I,J,K)*W(I,J,K);
					  M%AVG_T2(I,J,K) = TMP(I,J,K)*TMP(I,J,K);
				  END DO
			  END DO
		  END DO
...
! ENDDO
		  ...
		  DO k = 0,KBP1
			  DO j = 0,JBP1
				  DO i = 0,IBP1
					  M%AVG_T(I,J,K) = M%AVG_T(I,J,K) + TMP(I,J,K);
					  M%AVG_U(I,J,K) = M%AVG_U(I,J,K) + U(I,J,K);
					  M%AVG_V(I,J,K) = M%AVG_V(I,J,K) + V(I,J,K);
					  M%AVG_W(I,J,K) = M%AVG_W(I,J,K) + W(I,J,K);
					  M%AVG_TW(I,J,K) = M%AVG_TW(I,J,K) + TMP(I,J,K)*W(I,J,K);
					  M%AVG_TU(I,J,K) = M%AVG_TU(I,J,K) + TMP(I,J,K)*U(I,J,K);
					  M%AVG_TV(I,J,K) = M%AVG_TV(I,J,K) + TMP(I,J,K)*V(I,J,K);
					  M%AVG_UV(I,J,K) = M%AVG_UV(I,J,K) + U(I,J,K)*V(I,J,K);
					  M%AVG_WV(I,J,K) = M%AVG_WV(I,J,K) + W(I,J,K)*V(I,J,K);
					  M%AVG_WU(I,J,K) = M%AVG_WU(I,J,K) + W(I,J,K)*U(I,J,K);
					  M%AVG_U2(I,J,K) = M%AVG_U2(I,J,K) + U(I,J,K)*U(I,J,K);
					  M%AVG_V2(I,J,K) = M%AVG_V2(I,J,K) + V(I,J,K)*V(I,J,K);
					  M%AVG_W2(I,J,K) = M%AVG_W2(I,J,K) + W(I,J,K)*W(I,J,K);
					  M%AVG_T2(I,J,K) = M%AVG_T2(I,J,K) + TMP(I,J,K)*TMP(I,J,K);
				  END DO
			  END DO
		  END DO
...
! DEALLOCATE(W_VEL_R)
  ...
  ! IF (mod(ITM,5) == 0) then
	  ...
	  ! WRITE(FN_PROFAVG(NM),'(A,A,I0,A,I0,A)') TRIM(CHID),'_profavg_',NM,'_',ITM,'.txt'

		  ! OPEN(LU_PROFAVG(NM),FILE=FN_PROFAVG(NM),FORM='FORMATTED',ACCESS='SEQUENTIAL',STATUS='REPLACE')

		  ! DO K = 0,KBP1
			  ! DO J = 0,JBP1
				  ! DO I = 0,IBP1
					  ! WRITE(LU_PROFAVG(NM),*) ICYC, X(I), Y(J), Z(K), M%AVG_T(I,J,K), M%AVG_U(I,J,K), M%AVG_V(I,J,K), M%AVG_W(I,J,K), &
												  ! M%AVG_TW(I,J,K), M%AVG_TU(I,J,K), M%AVG_TV(I,J,K), &
												  ! M%AVG_UV(I,J,K), M%AVG_WV(I,J,K), M%AVG_WU(I,J,K), &
												  ! M%AVG_U2(I,J,K), M%AVG_V2(I,J,K), M%AVG_W2(I,J,K), M%AVG_T2(I,J,K)
				  ! END DO
			  ! END DO
		  ! END DO
...
!> \brief Dump moisture data to a file
!> \param T Current time (s)
!> \param DT Current time step size (s)
!> \param NM Mesh number

SUBROUTINE DUMP_TEMP(T,DT,NM,DNI_input,RelH_input,ICYC,Heig)
...
REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: NM
doubleprecision, DIMENSION(90000,1), INTENT(IN) :: DNI_input, RelH_input
INTEGER, INTENT(IN) :: ICYC, Heig
INTEGER :: NOM,N
INTEGER :: I,J,K,IT, INP_N_layers
REAL(EB), POINTER, DIMENSION(:,:,:) :: TEMP_INPUT, VELO_INPUT, HUMI_INPUT, RAD_INPUT, PRES_INPUT
!REAL(EB), ALLOCATABLE, DIMENSION(:,:) :: PART_TEMP_MOIS
INTEGER :: GEOM_INDEX=-1,TRNF_INDEX=-1,INDEX2=0,Z_INDEX=-999,Y_INDEX=-999,MATL_INDEX=0,&
              PART_INDEX=0,VELO_INDEX=0,PIPE_INDEX=0,PROP_INDEX=0,REAC_INDEX=0, data_tot, data_tot1
REAL(EB), ALLOCATABLE, DIMENSION(:,:)  :: m_fin, q_fin, T_a_fin, T_m_fin, liq_fin, T_l_fin
REAL(EB) :: litter_mois,spec_humidity,air_temp,litter_temp,liq,liq_temp
!REAL(EB), ALLOCATABLE, DIMENSION(:,:) :: TEMP_SOLID
INTEGER :: num,ext,ITM
...
num = ICYC;
INP_N_layers = N_layers;
! OPEN(LU_TEMP(NM),FILE=FN_TEMP(NM),FORM='FORMATTED',ACCESS='SEQUENTIAL',STATUS='REPLACE')
! OPEN(LU_VELO(NM),FILE=FN_VELO(NM),FORM='FORMATTED',ACCESS='SEQUENTIAL',STATUS='REPLACE')
...
ALLOCATE (TEMP_INPUT(IBAR,JBAR,data_tot))
ALLOCATE (VELO_INPUT(IBAR,JBAR,data_tot))
ALLOCATE (HUMI_INPUT(IBAR,JBAR,data_tot))
ALLOCATE (PRES_INPUT(IBAR,JBAR,data_tot))
!ALLOCATE (TEMP_SOLID(data_tot,1))
!ALLOCATE (QP(data_tot,1))
!ALLOCATE (PART_TEMP_MOIS(data_tot,1)) 

 DO J=1,JBAR
    DO I=1,IBAR
		TEMP_INPUT(I,J,Heig) = GAS_PHASE_OUTPUT(T,DT,NM,I,J,Heig,5,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,&
                                              PROP_INDEX,REAC_INDEX,MATL_INDEX);
		VELO_INPUT(I,J,Heig) = GAS_PHASE_OUTPUT(T,DT,NM,I,J,Heig,10,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,&
                                              PROP_INDEX,REAC_INDEX,MATL_INDEX);	
		HUMI_INPUT(I,J,Heig) = GAS_PHASE_OUTPUT(T,DT,NM,I,J,Heig,21,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,&
                                              PROP_INDEX,REAC_INDEX,MATL_INDEX);	
		!PRES_INPUT(I,J,Heig) = GAS_PHASE_OUTPUT(T,DT,NM,I,J,Heig,523,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,&
         !                                     PROP_INDEX,REAC_INDEX,MATL_INDEX);
		!TEMP_SOLID(I,J,1) = M%TEMP_SURF(I,J,1,1)
	ENDDO
 ENDDO

! DO I = 1,data_tot
		! TEMP_SOLID(I,1) = M%TEMP_SURF(I,1)
		
		! !print*, T, NM, I, TEMP_SOLID(I,1)
! ENDDO
ITM = INT(T)

!Call DUMP_PART(T,NM)

!print*, PART_TEMP_MOIS

!IF (MOD(ITM,1) == 0) THEN
!OPEN(LU_TEMP(NM),FILE=FN_TEMP(NM),FORM='FORMATTED',ACCESS='SEQUENTIAL',STATUS='REPLACE')
!DO J= 1,JBAR
!	DO I= 1,IBAR
!		WRITE(LU_TEMP(NM),*) X(I), Y(J), PART_TEMP_		
!	ENDDO
!ENDDO
!CLOSE(LU_TEMP(NM))
!ENDIF

CALL MoistureModel (T, DT, NM, data_tot, num, Heig, DNI_input, RelH_input,TEMP_INPUT,VELO_INPUT,HUMI_INPUT,PRES_INPUT,INP_N_layers)

DEALLOCATE (TEMP_INPUT)
DEALLOCATE (VELO_INPUT)
DEALLOCATE (HUMI_INPUT)
DEALLOCATE (PRES_INPUT)
!DEALLOCATE (TEMP_SOLID)

END SUBROUTINE DUMP_TEMP

!> \physics-based dynamic FMC model 
SUBROUTINE MoistureModel (T, DT, NM, data_tot, num, Heig, DNI_input, RelH_input,TEMP_INPUT,VELO_INPUT,HUMI_INPUT,PRES_INPUT,INP_N_layers)

IMPLICIT NONE

REAL(EB), INTENT(IN) :: T,DT
INTEGER, INTENT(IN) :: NM, data_tot, num, Heig,INP_N_layers
REAL(EB), DIMENSION(90000,1), INTENT(IN) :: DNI_input, RelH_input
REAL(EB), DIMENSION(IBAR,JBAR,data_tot), INTENT(IN) :: TEMP_INPUT, VELO_INPUT, HUMI_INPUT, PRES_INPUT
!REAL(EB), DIMENSION(data_tot,1), INTENT(IN) :: TEMP_SOLID
REAL(EB), DIMENSION(data_tot,Inp_N_layers) :: m_fin, q_fin, T_a_fin, T_m_fin, liq_fin, T_l_fin
REAL(EB), DIMENSION(data_tot) :: T_s_all, T_s_last_ten
REAL(EB) :: q(Inp_N_layers,1), T_m(Inp_N_layers,1), mo(Inp_N_layers,1), T_a(Inp_N_layers,1), liq(Inp_N_layers,1), T_l(Inp_N_layers,1)
INTEGER :: i
INTEGER :: j
INTEGER :: k
INTEGER :: ST, it_inside

REAL(EB), allocatable, dimension(:) :: temp_a_all, wind_all, RH_calc_all, DNI_save, pres_all, surf_temp_all

REAL(EB), allocatable, dimension(:,:) :: temp_a_all_read, wind_all_read
REAL(EB) :: temp_a, DNI, RelH, pressure, wind, rain, T_s, temp_a_in, RelH_in, &
				delta_t, delta_rh, wind_in, delta_wind, dt_dec, delta_rad, rad_in, surf_temp_in

REAL(EB) :: f1, f2, f3, f4, f5, f6S
REAL(EB) :: m_old(Inp_N_layers,1), q_old(Inp_N_layers,1), T_a_old(Inp_N_layers,1), T_m_old(Inp_N_layers,1), &
				liq_old(Inp_N_layers,1), T_liq_old(Inp_N_layers,1)
REAL(EB) :: f_m1, f_q1, f_Ta1, f_Tm1, F(6,Inp_N_layers), F1_new(6,Inp_N_layers), f_li1, f_li2, f_tliq1, f_tliq2
REAL(EB) :: f_old(6,Inp_N_layers), f_old1(6,Inp_N_layers)
REAL(EB) :: f_new(6,Inp_N_layers), f_new1(6,Inp_N_layers)
REAL(EB) :: dh1, dh2, dh3, dh4
integer :: data_num, it, itera_m(Inp_N_layers,1), itera_q(Inp_N_layers,1), itera_ta(Inp_N_layers,1), itera_tm(Inp_N_layers,1), &
				itera_l(Inp_N_layers,1), itera_tl(Inp_N_layers,1), local_index
REAL(EB) :: al(6,Inp_N_layers), Y_old(6,Inp_N_layers), Y_guess(6,Inp_N_layers), Y_new(6,Inp_N_layers)
REAL(EB) :: tol, error1, err_mat(6,Inp_N_layers)
REAL(EB) :: dfm1, dq1, dTa1, dTm1, dfm2, dq2, dTa2, dliq1, dTl1, dTm2, dliq2, dTl2, dfm3, dq3, dTa3, dTm3, dliq3, dTl3
REAL(EB) :: dfm4, dq4, dTa4, dTm4, dfm5, dq5, dTa5, dTm5
REAL(EB) :: err_m(Inp_N_layers,1), err_q(Inp_N_layers,1), err_ta(Inp_N_layers,1), err_tm(Inp_N_layers,1), err_l(Inp_N_layers,1), err_tl(Inp_N_layers,1)
INTEGER :: max_itera, Ntotal, ITM, ll_count, division, soil_it
REAL(EB), allocatable, dimension(:,:) :: m_at_old_t, q_at_old_t, T_a_at_old_t, T_m_at_old_t, &
											liq_at_old_t, T_l_at_old_t									
REAL(EB), DIMENSION(6,1) :: Inp, TT
INTEGER, allocatable, dimension(:,:) :: ki
REAL(EB), allocatable, dimension(:) :: mo_accumu, mo_ave
TYPE (MESH_TYPE), POINTER :: M
REAL(EB), DIMENSION(Inp_N_layers,1) :: q_sat_check
REAL(EB), allocatable, DIMENSION(:) :: lambda_max
REAL(EB) :: lambda_all(Inp_N_layers,1)

allocate(temp_a_all(data_tot))
allocate(wind_all(data_tot))

allocate(m_at_old_t(data_tot,Inp_N_layers))
allocate(q_at_old_t(data_tot,Inp_N_layers))
allocate(T_a_at_old_t(data_tot,Inp_N_layers))
allocate(T_m_at_old_t(data_tot,Inp_N_layers))
allocate(liq_at_old_t(data_tot,Inp_N_layers))
allocate(T_l_at_old_t(data_tot,Inp_N_layers))

allocate(RH_calc_all(data_tot))
allocate(mo_accumu(data_tot))
allocate(mo_ave(data_tot))
allocate(DNI_save(data_tot))
allocate(pres_all(data_tot))
allocate(surf_temp_all(data_tot))

allocate(lambda_max(data_tot))
...
open (110221, file='Input.txt', FORM='FORMATTED', ACCESS='SEQUENTIAL', STATUS='OLD', ACTION='READ')
read (110221,*) Inp
close (110221)

do it = 1, data_tot
	do i = 1,Inp_N_layers
		m_at_old_t(it,i) = Inp(1,1);
		q_at_old_t(it,i) = Inp(2,1);
		T_a_at_old_t(it,i) = Inp(3,1);
		T_m_at_old_t(it,i) = Inp(4,1);
		liq_at_old_t(it,i) = Inp(5,1);
		T_l_at_old_t(it,i) = Inp(6,1);
	end do
end do
...
do j = 1, data_tot
	do i = 1,Inp_N_layers
		...
		if(M%koba_m_fin(j,i) .le. 0.0001 .or. M%koba_m_fin(j,i) == 0.0001)then
			m_at_old_t(j,i) = 0.0002
		else
			m_at_old_t(j,i) = M%koba_m_fin(j,i);
		endif	
		
		!m_at_old_t(j,i) = M%koba_m_fin(j,i)
		q_at_old_t(j,i) = M%koba_q_fin(j,i)
		T_a_at_old_t(j,i) = M%koba_ta_fin(j,i)
		T_m_at_old_t(j,i) = M%koba_tm_fin(j,i)
		!liq_at_old_t(j,i) = M%koba_l_fin(j,i)
		!T_l_at_old_t(j,i) = M%koba_tl_fin(j,i)
	end do
end do
...
! do J = 1,JBAR
	! do I = 1,IBAR
		! M%koba_soiltemp(ki(I,J),num) = 0;
	! end do
! end do

! IF (num .LE. 20) then						!! Update the soil temperature matrix for 10 iterations
		! do J = 1,JBAR
			! do I = 1,IBAR
					
					! IF (num == 1) THEN
						! M%koba_soiltemp(ki(I,J),num) = 21.0;
					! ELSE
						! M%koba_soiltemp(ki(I,J),num) = ((M%koba_tm_fin(ki(I,J),1)+M%koba_tm_fin(ki(I,J),2)+ &
								! M%koba_tm_fin(ki(I,J),3)+M%koba_tm_fin(ki(I,J),4)+M%koba_tm_fin(ki(I,J),5))/5) - 273.0;
					! ENDIF
							
						! T_s_all(ki(I,J)) = 21.0;			!! Keep it constant for 20 iterations 
			! end do
		! end do	
! ELSE
		! do J = 1,JBAR
			! do I = 1,IBAR
				! T_s_all(ki(I,J)) = (M%koba_soiltemp(ki(I,J),1)+M%koba_soiltemp(ki(I,J),2)+M%koba_soiltemp(ki(I,J),3)+ &
							! M%koba_soiltemp(ki(I,J),4)+M%koba_soiltemp(ki(I,J),5)+M%koba_soiltemp(ki(I,J),6)+ M%koba_soiltemp(ki(I,J),7)+ &
							! M%koba_soiltemp(ki(I,J),8)+M%koba_soiltemp(ki(I,J),9)+M%koba_soiltemp(ki(I,J),10)+ &
							! M%koba_soiltemp(ki(I,J),11)+M%koba_soiltemp(ki(I,J),12)+M%koba_soiltemp(ki(I,J),13)+ &
							! M%koba_soiltemp(ki(I,J),14)+M%koba_soiltemp(ki(I,J),15)+M%koba_soiltemp(ki(I,J),16)+ M%koba_soiltemp(ki(I,J),17)+ &
							! M%koba_soiltemp(ki(I,J),18)+M%koba_soiltemp(ki(I,J),19)+M%koba_soiltemp(ki(I,J),20))/20;
				! !! change the values of the columns to remove old values and add new values
				! do ST = 1,19
					! M%koba_soiltemp(ki(I,J),ST) = M%koba_soiltemp(ki(I,J),ST+1);
				! end do

					! ! do soil_it = 1,INP_N_LAYERS
						! ! M%koba_soiltemp(ki(I,J),num) = M%koba_soiltemp(ki(I,J),num) + M%koba_tm_fin(ki(I,J),soil_it)
					! ! end do
					
					! ! M%koba_soiltemp(ki(I,J),num) = (M%koba_soiltemp(ki(I,J),num)/INP_N_LAYERS) - 273.0;
...
do J = 1,JBAR
	do I = 1,IBAR
		temp_a_all(ki(I,J)) = TEMP_INPUT(I,J,Heig)
		wind_all(ki(I,J)) = VELO_INPUT(I,J,Heig)
		RH_calc_all(ki(I,J)) = HUMI_INPUT(I,J,Heig)
		pres_all(ki(I,J)) = PRES_INPUT(I,J,Heig)
		surf_temp_all(ki(I,J)) = TEMP_INPUT(I,J,Heig)
	end do
end do

do J = 1,JBAR
	do I = 1,IBAR
		T_s_all(ki(I,J)) = temp_a_all(ki(I,J));
	end do
end do
...
do J = 1,JBAR
	do I = 1,IBAR
		WRITE(LU_TEMP(NM),*) X(I), Y(J), RH_calc_all(ki(I,J))
	end do
end do
...
temp_a = temp_a_all(data_num);
wind = ABS(wind_all(data_num));
! DNI = DNI_input(num,1)
! if (M%koba_tl_fin(data_num,1) .LT. 0) then
	! DNI = DNI_input(num,1)
! else
DNI = DNI_input(num,1)
!+ abs(M%koba_tl_fin(data_num,1)/1000.0);
! endif
!DNI = M%koba_tl_fin(data_num,1);
RelH = RH_calc_all(data_num);
T_s = T_s_all(data_num);
pressure = 1000;
rain = 0
surf_temp_in = surf_temp_all(data_num)

do i = 1,data_tot
	DNI_save(i) = DNI_input(num,1) + M%koba_tl_fin(num,5);
enddo
...
do while (it <= Ntotal)
			...
			temp_a = temp_a_all(data_num)
			DNI = DNI_input(num,1) + M%koba_tl_fin(num,5)
			RelH = RH_calc_all(data_num)
			wind = wind_all(data_num)
			T_s = T_s_all(data_num);
			surf_temp_in = surf_temp_all(data_num)
			
			! print*, DNI_input(num,1), M%koba_tl_fin(num,5)
			
			do i = 1,Inp_N_layers
			   
				m_old(i,1) = m_at_old_t(it,i)
				q_old(i,1) = q_at_old_t(it,i)
				T_a_old(i,1) = T_a_at_old_t(it,i)
				T_m_old(i,1) = T_m_at_old_t(it,i)
				liq_old(i,1) = liq_at_old_t(it,i)
				T_liq_old(i,1) = T_l_at_old_t(it,i)
	...
	! call stability(T,mo,q,T_a,T_m,m_old,T_m_old,liq,T_l,dt,wind,DNI,temp_a,P_INF,RelH,rain,T_s,surf_temp_in,local_index,INP_N_layers,lambda_all)
	
	! lambda_max(it) = 2/(maxval(lambda_all))
	
	!print*, NM, it, dt, lambda_max(it), lambda_all
		...
		data_num = it;
			...
			wind = wind_all(data_num)
			T_s = T_s_all(data_num);
			! pressure = pres_all(data_num)*100000
			surf_temp_in = surf_temp_all(data_num)
			error1 = 1;
			!print*, surf_temp_in
			do i = 1,Inp_N_layers
			...
			do local_index = 1,Inp_N_layers
				...
				do while (err_ta(local_index,1) > tol)
					...
					call vector_f(mo,q,T_a,T_m,m_old,q_old,T_a_old,T_m_old,liq,T_l,liq_old,T_liq_old,dt,wind,DNI,temp_a,pressure,RelH,rain,T_s,INP_N_layers,&
					local_index,F);
					...
					call f_olda(mo,q,T_a,T_m,m_old,q_old,T_a_old,T_m_old,liq,T_l,liq_old,T_liq_old,dt,wind,DNI,temp_a,pressure,RelH,rain,T_s,INP_N_layers,&
					local_index,f_old);
			...
			do local_index = 1,Inp_N_layers
				...
				do while (err_q(local_index,1) > tol)
					...
					!!!!!!!!!!!! Commenting this on 08/21/2022

					q_sat_check(local_index,1) = (0.622*0.611*(exp((17.3*(T_a(local_index,1)-273))/(T_a(local_index,1)-273+237.3))))/(pressure*0.1);

					if (Y_new(2,local_index) .GT. q_sat_check(local_index,1)) then

					Y_new(2,local_index) = q_sat_check(local_index,1);

					! elseif (Y_new(2,local_index) .GT. 1) then

               				     ! Y_new(2,local_index) = 1;

					elseif (Y_new(2,local_index) .LT. 1.E-5) then

					Y_new(2,local_index) = 0.00001
			...
			do local_index = 1,Inp_N_layers
				...
				do while (err_tm(local_index,1) > tol)
					...
					call vector_f(mo,q,T_a,T_m,m_old,q_old,T_a_old,T_m_old,liq,T_l,liq_old,T_liq_old,dt,wind,DNI,temp_a,pressure,RelH,rain,T_s,INP_N_layers,&
					local_index,F); ! Additionally output E_ma(T_m)
...
! do J = 1,JBAR
	! do I = 1,IBAR		
			! M%koba_tl_fin(ki(I,J),2) = temp_a_all(ki(I,J))
			! M%koba_tl_fin(ki(I,J),3) = RH_calc_all(ki(I,J))
			! M%koba_tl_fin(ki(I,J),4) = wind_all(ki(I,J))
			! M%koba_tl_fin(ki(I,J),5) = DNI_save(ki(I,J))
	! end do
! end do

do i = 1,data_tot
		do j = 1,Inp_N_layers
			M%koba_m_fin(i,j) = m_fin(i,j)
			M%koba_q_fin(i,j) = q_fin(i,j)
			M%koba_ta_fin(i,j) = T_a_fin(i,j)
			M%koba_tm_fin(i,j) = T_m_fin(i,j)
			!M%koba_l_fin(i,j) = liq_fin(i,j)
			!M%koba_tl_fin(i,j) = T_l_fin(i,j)
		end do
end do	

! do j = 1,JBAR
	! do i = 1,IBAR
				! M%koba_soiltemp(ki(I,J),20) = ((M%koba_tm_fin(ki(I,J),1)+M%koba_tm_fin(ki(I,J),2)+ &
							! M%koba_tm_fin(ki(I,J),3)+M%koba_tm_fin(ki(I,J),4)+M%koba_tm_fin(ki(I,J),5))/5) - 273.0;
	! end do
! end do
...
ITM  = INT(T)
 ...
 IF (mod(ITM,5) == 0) then
	 ...
	 WRITE(FN_MOISTURE(NM,Inp_N_layers),'(A,A,I0,A,I0,A)') TRIM(CHID),'_Mois_',NM,'_',ITM,'.txt'
	 WRITE(FN_SPEC_HUM(NM,Inp_N_layers),'(A,A,I0,A,I0,A)') TRIM(CHID),'_SpecHumi_',NM,'_',ITM,'.txt'
	 WRITE(FN_TEMP_A(NM,Inp_N_layers),'(A,A,I0,A,I0,A)') TRIM(CHID),'_AirTemp_',NM,'_',ITM,'.txt'
	 WRITE(FN_TEMP_M(NM,Inp_N_layers),'(A,A,I0,A,I0,A)') TRIM(CHID),'_LitterTemp_',NM,'_',ITM,'.txt'
	 WRITE(FN_LIQ(NM,Inp_N_layers),'(A,A,I0,A,I0,A)') TRIM(CHID),'_Liq_',NM,'_',ITM,'.txt'
	 WRITE(FN_TEMP_L(NM,Inp_N_layers),'(A,A,I0,A,I0,A)') TRIM(CHID),'_Temp_l_',NM,'_',ITM,'.txt'

	 OPEN(LU_MOISTURE(NM,Inp_N_layers),FILE=FN_MOISTURE(NM,Inp_N_layers),FORM='FORMATTED',ACCESS='SEQUENTIAL',STATUS='REPLACE')
		 DO it = 1,Ntotal
				 WRITE(LU_MOISTURE(NM,Inp_N_layers),*) m_fin(it,:)
				! !WRITE(LU_MOISTURE(NM,5),"(32(f8.3,',',:))") m_at_old_t(it,1), m_at_old_t(it,2), m_at_old_t(it,3), &
				! !												m_at_old_t(it,4), m_at_old_t(it,5)
		 ENDDO
	 CLOSE(LU_MOISTURE(NM,Inp_N_layers))

	 OPEN(LU_SPEC_HUM(NM,Inp_N_layers),FILE=FN_SPEC_HUM(NM,Inp_N_layers),FORM='FORMATTED',ACCESS='SEQUENTIAL',STATUS='REPLACE')
		 DO it = 1,Ntotal
				 WRITE(LU_SPEC_HUM(NM,Inp_N_layers),*) q_fin(it,:)
				! !WRITE(LU_SPEC_HUM(NM,Inp_N_layers),"(32(f8.3,',',:))") wind_all(it) 
		 ENDDO
	 CLOSE(LU_SPEC_HUM(NM,Inp_N_layers))

	 OPEN(LU_TEMP_A(NM,Inp_N_layers),FILE=FN_TEMP_A(NM,Inp_N_layers),FORM='FORMATTED',ACCESS='SEQUENTIAL',STATUS='REPLACE')
		 DO it = 1,Ntotal
				 WRITE(LU_TEMP_A(NM,Inp_N_layers),*) T_a_fin(it,:)
		 ENDDO
	 CLOSE(LU_TEMP_A(NM,Inp_N_layers))

	 OPEN(LU_TEMP_M(NM,Inp_N_layers),FILE=FN_TEMP_M(NM,Inp_N_layers),FORM='FORMATTED',ACCESS='SEQUENTIAL',STATUS='REPLACE')
		 DO it = 1,Ntotal
				 WRITE(LU_TEMP_M(NM,Inp_N_layers),*) T_m_fin(it,:)
		 ENDDO
	 CLOSE(LU_TEMP_M(NM,Inp_N_layers))

	 OPEN(LU_LIQ(NM,Inp_N_layers),FILE=FN_LIQ(NM,Inp_N_layers),FORM='FORMATTED',ACCESS='SEQUENTIAL',STATUS='REPLACE')
		 DO it = 1,Ntotal
				 WRITE(LU_LIQ(NM,Inp_N_layers),*) RH_calc_all(it), temp_a_all(it), wind_all(it), DNI_save(it), T_s_all(it), pres_all(it), surf_temp_all(it)
		 ENDDO
	 CLOSE(LU_LIQ(NM,Inp_N_layers))

	 OPEN(LU_TEMP_L(NM,Inp_N_layers),FILE=FN_TEMP_L(NM,Inp_N_layers),FORM='FORMATTED',ACCESS='SEQUENTIAL',STATUS='REPLACE')
		 DO it = 1,Ntotal
				 WRITE(LU_TEMP_L(NM,Inp_N_layers),*) M%koba_tl_fin(it,1), M%koba_tl_fin(it,2), M%koba_tl_fin(it,3), M%koba_tl_fin(it,4)
		 ENDDO
	 CLOSE(LU_TEMP_L(NM,Inp_N_layers))

END IF

deallocate(temp_a_all)
deallocate(wind_all)

deallocate(m_at_old_t)
deallocate(q_at_old_t)
deallocate(T_a_at_old_t)
deallocate(T_m_at_old_t)
deallocate(liq_at_old_t)
deallocate(T_l_at_old_t)

deallocate(RH_calc_all)
deallocate(mo_accumu)
deallocate(mo_ave)
deallocate(DNI_save)
deallocate(pres_all)
deallocate(surf_temp_all)

deallocate(ki)

END subroutine MoistureModel


! Import number of layers Inp_N_layers
! Put a generalize function or maybe an index i that knows the value

subroutine f_m_calc_1a(mo,q,T_a,T_m,m_old,liq,T_l,dt,wind,DNI,temp_a,P_INF,RelH,rain,T_s,local_index,INP_N_layers,f_m1)
IMPLICIT NONE

REAL(EB), intent(in) :: mo(Inp_N_layers,1), q(Inp_N_layers,1), T_a(Inp_N_layers,1), T_m(Inp_N_layers,1), &
									m_old(Inp_N_layers,1), liq(Inp_N_layers,1), T_l(Inp_N_layers,1)
!integer, intent(in) :: data_num
REAL(EB), INTENT(IN) :: DT
REAL(EB), intent(in) :: wind, DNI, temp_a, P_INF, RelH, rain, T_s
INTEGER, INTENT(IN) :: local_index,INP_N_layers
REAL(EB), intent(out) :: f_m1
...
doubleprecision :: mu_fmc_ma,mu_fmc_ml, mu_fmc_la, mu_fmc_ml1, mu_fmc_la1, &
					S_lma, S_ml, S_ma, S_la, Drain, D_p, C_hl, V_l, q0_sat
doubleprecision :: check(2,1)
doubleprecision :: mo_local, T_m_local, q_local, T_a_local, T_l_local, l_local
!!!! Moisture model parameter
REAL(EB), PARAMETER :: rho_litter = 593						! litter density
!REAL(EB), PARAMETER :: R = 8.314								! universal gas constant
REAL(EB), PARAMETER :: Mass_fmc = 18.0153							! molecular mass of water
REAL(EB), PARAMETER :: A_fmc = 5.2								! Nelson model constant
REAL(EB), PARAMETER :: B_fmc = -19.0								! Nelson model constant
!REAL(EB), PARAMETER :: P = 100000.0
REAL(EB), PARAMETER :: T_ref = 273.16

REAL(EB), PARAMETER :: rho_air = 1.2

REAL(EB), PARAMETER :: gama = 1.363
REAL(EB), PARAMETER :: hei = 0.02
REAL(EB), PARAMETER :: alpha_s = 0.2
!REAL(EB), PARAMETER :: sigma_st = 5.67/(10**(8))
REAL(EB), PARAMETER :: alpha_l = 0.27

REAL(EB), PARAMETER :: mu_fmc = 4550.0
REAL(EB), PARAMETER :: K_maE = 0.006
REAL(EB), PARAMETER :: rho_bulk =62.00
REAL(EB), PARAMETER :: C_p = 1004.5
REAL(EB), PARAMETER :: lambda = 2.45*1000000
REAL(EB), PARAMETER :: K_c = 0.2
REAL(EB), PARAMETER :: tau_i0 = 1.0

REAL(EB), PARAMETER :: D_T0a = 0.00002
REAL(EB), PARAMETER :: D_T0b = 2.6
REAL(EB), PARAMETER :: zeta_a = 2.08
REAL(EB), PARAMETER :: zeta_b = 2.38
REAL(EB), PARAMETER :: d_fmc = 0.03

REAL(EB), PARAMETER :: z_screen = 0.042
REAL(EB), PARAMETER :: z_0 = 0.01
REAL(EB), PARAMETER :: K_von = 0.4

REAL(EB), PARAMETER :: C_hm = 1.0*1000000
REAL(EB), PARAMETER :: kappa_h = 2.08
REAL(EB), PARAMETER :: kappa_v = 2.34

REAL(EB), PARAMETER :: rho_water = 1000.0

REAL(EB), PARAMETER :: D_s = 1.153
REAL(EB), PARAMETER :: D_a = 0.00003
REAL(EB), PARAMETER :: L_a = 0.23;
REAL(EB), PARAMETER :: L_b = -1.63;
REAL(EB), PARAMETER :: K_mlH = 700.0;
REAL(EB), PARAMETER :: R0=8314.472                       !< Gas constant (J/K/kmol)
REAL(EB), PARAMETER :: SIGMA=5.670373/100000000                 !< Stefan-Boltzmann
!REAL(EB), PARAMETER :: LAI = 1._EB				                 !< Leaf area index
REAL(EB), PARAMETER :: LAI = 0.5				                 ! CC = 94.00*[1-exp(-0.43*LAI)]^(0.52)
...
mu_fmc_ml = 0;
mu_fmc_ml1 = 0;
!print*, 'fm1 1', dz, S_lma, dz, S_ma, V_m, V_a
calc = exp((mo(local_index,1)*B_fmc) + A_fmc);

RH_fuel = exp((-4.19*Mass_fmc*calc)/((R0/1000)*T_m(local_index,1)));
q_sat = (0.622*0.611*(exp((17.3*(T_m(local_index,1)-273))/(T_m(local_index,1)-273+237.3))))/(P_INF*0.1);
q0 = (RelH*exp((17.67*((temp_a+273)-T_ref))/((temp_a+273)-29.65)))/(0.263*P_INF*100);
q0_sat = (0.622*0.611*(exp((17.3*(temp_aK-273))/(temp_aK-273+237.3))))/(P_INF*0.1);

! !print*, q0_sat, q0
! if (q0_sat .LT. q0) then
    ! q0 = q0_sat
! end if
...
doubleprecision :: mu_fmc_ma,mu_fmc_ml, mu_fmc_la, mu_fmc_ml1, mu_fmc_la1,S_lma, &
					S_ml, S_ma, S_la, Drain, D_p, C_hl, V_l, q0_sat
doubleprecision :: check(2,1)
!!!! Moisture model parameter
REAL(EB), PARAMETER :: rho_litter = 593						! litter density
!REAL(EB), PARAMETER :: R = 8.314								! universal gas constant
...
elseif (local_index == Inp_N_layers) then
	...
	!E_T = 0.0;
	E_T = rho_air*D_T0*exp(zeta*((((local_index-1)*hei)/((Inp_N_layers)*hei))-1))*((0.0)-q(local_index,1))/(dz/2);
...
q_sat = (0.622*0.611*(exp((17.3*(T_m(1,1)-273))/(T_m(1,1)-273+237.3))))/(P_INF*0.1);
...
!doubleprecision :: S_dn, L_up0, S_up0, tau_i, S_up, L_dn, L_up, R_net, K_maH, H_c, D_T

!doubleprecision :: E_ma, E_T0, E_T, H_T0, H_T, H_ma, R_net0, mu_fmc_ma1, T_s, K_T0, temp_aK
...
doubleprecision :: E_ma, E_T0, E_T(Inp_N_layers,1), H_T0, H_T(Inp_N_layers,1), H_ma, R_net0, mu_fmc_ma1, K_T0, temp_aK, &
						S_dn_st, L_up_st, L_dn_st, q0_sat
...
q_sat(i,1) = (0.622*0.611*(exp((17.3*(T_m(i,1)-273))/(T_m(i,1)-273+237.3))))/(P_INF*0.1);
end do
...
REAL(EB), intent(in) :: wind, DNI, temp_a, P_INF, RelH,rain,T_s
REAL(EB), intent(out) :: f_old(6,Inp_N_layers)
...
doubleprecision :: RiB(Inp_N_layers,1)
doubleprecision :: zama_h
doubleprecision :: gamma_h(Inp_N_layers,1)
doubleprecision :: q0
...
do i = 1,Inp_N_layers
	...
	if (i == 1) then
		...
		!E_T(i,1) = 0.0;
		E_T(i,1) = rho_air*D_T0*exp(zeta*(((Inp_N_layers*hei/(Inp_N_layers))/hei)-1))*((0.0)-q(i,1))/(0.5*dz);
...
do i = 1,Inp_N_layers
	...
	if (i == 1) then
		...
		!H_c(i,1) = (0.14+0.22*mo(local_index,1))*(T_m(i+1,1)-T_m(i,1))/dz;
		!H_C0(i,1) = (0.14+0.22*mo(local_index,1))*(T_m(i,1)-temp_aK)/(0.5*dz);	! Should be dz/2
	...
	elseif (i == Inp_N_layers) then
		...
		!H_c(i,1) = (0.14+0.22*mo(local_index,1))*(T_s+273-T_m(i,1))/(0.5*dz);	! Should be dz/2
		!H_C0(i,1) = (0.14+0.22*mo(local_index,1))*(T_m(i,1)-T_m(i-1,1))/dz;
...
REAL(EB), intent(in) :: wind, DNI, temp_a, P_INF, RelH, rain, T_s
INTEGER, intent(in) :: INP_N_layers,local_index
REAL(EB), intent(out) :: F(6,Inp_N_layers)
...
IF (LEVEL_SET_MODE>0) WRITE(LU_CORE(NM)) PHI_LS
	...
	WRITE(LU_CORE(NM)) M%AVG_T
	WRITE(LU_CORE(NM)) M%AVG_U
	WRITE(LU_CORE(NM)) M%AVG_V
	WRITE(LU_CORE(NM)) M%AVG_W
	WRITE(LU_CORE(NM)) M%AVG_TW
	WRITE(LU_CORE(NM)) M%AVG_TU
	WRITE(LU_CORE(NM)) M%AVG_TV
	WRITE(LU_CORE(NM)) M%AVG_UV
	WRITE(LU_CORE(NM)) M%AVG_WV
	WRITE(LU_CORE(NM)) M%AVG_WU
	WRITE(LU_CORE(NM)) M%AVG_U2
	WRITE(LU_CORE(NM)) M%AVG_V2
	WRITE(LU_CORE(NM)) M%AVG_W2
	WRITE(LU_CORE(NM)) M%AVG_T2
	
	WRITE(LU_CORE(NM)) M%koba_m_fin
	WRITE(LU_CORE(NM)) M%koba_q_fin
	WRITE(LU_CORE(NM)) M%koba_ta_fin
	WRITE(LU_CORE(NM)) M%koba_tm_fin
	WRITE(LU_CORE(NM)) M%koba_l_fin
	WRITE(LU_CORE(NM)) M%koba_tl_fin
	
!	WRITE(LU_CORE(NM)) M%TEMP_SURF
...
IF (LEVEL_SET_MODE>0) READ(LU_RESTART(NM)) PHI_LS
	...
	READ(LU_RESTART(NM)) M%AVG_T
	READ(LU_RESTART(NM)) M%AVG_U
	READ(LU_RESTART(NM)) M%AVG_V
	READ(LU_RESTART(NM)) M%AVG_W
	READ(LU_RESTART(NM)) M%AVG_TW
	READ(LU_RESTART(NM)) M%AVG_TU
	READ(LU_RESTART(NM)) M%AVG_TV
	READ(LU_RESTART(NM)) M%AVG_UV
	READ(LU_RESTART(NM)) M%AVG_WV
	READ(LU_RESTART(NM)) M%AVG_WU
	READ(LU_RESTART(NM)) M%AVG_U2
	READ(LU_RESTART(NM)) M%AVG_V2
	READ(LU_RESTART(NM)) M%AVG_W2
	READ(LU_RESTART(NM)) M%AVG_T2
	
	READ(LU_RESTART(NM)) M%koba_m_fin
	READ(LU_RESTART(NM)) M%koba_q_fin
	READ(LU_RESTART(NM)) M%koba_ta_fin
	READ(LU_RESTART(NM)) M%koba_tm_fin
	READ(LU_RESTART(NM)) M%koba_l_fin
	READ(LU_RESTART(NM)) M%koba_tl_fin
	
!	READ(LU_RESTART(NM)) M%TEMP_SURF
...
INTEGER, INTENT(IN)  :: NM
REAL(EB), INTENT(IN) :: T
REAL(EB), POINTER, DIMENSION(:,:) :: PART_TEMP_MOIS
REAL(EB) :: STIME
...
INTEGER :: N_START, N_END, I_WRITE, J_WRITE
...
LAGRANGIAN_PARTICLE_CLASS_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   ! ALLOCATE(PART_TEMP_MOIS(IBAR*JBAR,1),STAT=IZERO)   			 ; CALL ChkMemErr('DUMP','PART_TEMP_MOIS',IZERO)
  ! ALLOCATE(PART_TEMP_MOIS(IBAR*JBAR,1));
   ...
   LOAD_LOOP: DO IP=1,NLP
	  ...
	  ! IF (NM .GE. 2 .AND. NM .LE. 6) THEN
		! IF (IP .GE. N_START .AND. IP .LE. N_END) THEN
			! I_WRITE = IP - J_WRITE + 1 ;
			! PART_TEMP_MOIS(I_WRITE,1) = PARTICLE_OUTPUT(T,437,IP,&
					! Y_INDEX=LPC%QUANTITIES_Y_INDEX(1),Z_INDEX=LPC%QUANTITIES_Z_INDEX(1),MESH_NUMBER=NM)
			! M%TEMP_SURF(I_WRITE,1) = PART_TEMP_MOIS(I_WRITE,1);
			! !print*, T, NM, XP(NPP), YP(NPP), ZP(NPP), PART_TEMP_MOIS(I_WRITE,1)
		! ENDIF
	  ! ENDIF
   ENDDO LOAD_LOOP
...
subroutine test(dt, T, T_out)

REAL(EB), INTENT(IN) :: dt, T
REAL(EB), INTENT(OUT) :: T_out
