...
INTEGER :: start_save, end_save, data_tot, INT_var
REAL(EB), allocatable, dimension(:) :: RAD_to_FMC_model

CALL POINT_TO_MESH(NM)
M => MESHES(NM)
		
ALLOCATE(Z_ARRAY(N_TRACKED_SPECIES))
allocate(RAD_to_FMC_model(IBAR*JBAR))
...
BAND_LOOP: DO IBND = 1,NUMBER_SPECTRAL_BANDS
   ...
   INTENSITY_UPDATE: IF (UPDATE_INTENSITY) THEN
      ...
      ! Compute boundary condition term incoming radiation integral
		...
		start_save = 2*(IBAR*KBAR)+2*(JBAR*KBAR)
		end_save = start_save + (IBAR*JBAR)
      ...
      DO IW=1,N_EXTERNAL_WALL_CELLS+N_INTERNAL_WALL_CELLS
         ...
         WALL(IW)%ONE_D%Q_RAD_IN  = WALL(IW)%ONE_D%Q_RAD_IN + WALL(IW)%ONE_D%EMISSIVITY*(INRAD_W(IW)+BBFA*EFLUX)
			...
			IF (IW .GT. start_save) THEN
				IF (IW .LT. end_save) THEN
					RAD_to_FMC_model(IW-start_save) = WALL(IW)%ONE_D%Q_RAD_IN
				ENDIF
			ENDIF
      ENDDO
	  
	  	DO INT_var = 1,(IBAR*JBAR)
			M%koba_tl_fin(INT_var,1) = RAD_to_FMC_model(INT_var)
		ENDDO		
